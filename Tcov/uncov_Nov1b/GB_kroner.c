...
// covered (3437): 3362896
        ...
        for (kC = 0 ; kC < cnvec ; kC++)
        {
            ...
            if (!C_is_full)
            {   GB_cov[3438]++ ;
// covered (3438): 26999215
                Cp [kC] = aknz * bknz ;
            }
            if (C_is_hyper)
            {   GB_cov[3439]++ ;
// covered (3439): 10094968
                Ch [kC] = jA * bvdim + jB ;
            }
        }
...
// covered (3440): 42795851
                ...
                if (op_is_positional)
                {
                    ...
                    switch (opcode)
                    {
                        case GB_FIRSTI_opcode    : GB_cov[3441]++ ;  
// covered (3441): 3824363
                            // z = first_i(A(iA,jA),y) == iA
                        case GB_FIRSTI1_opcode   : GB_cov[3442]++ ;  
// covered (3442): 3861879
                            // z = first_i1(A(iA,jA),y) == iA+1
                            if (is64)
                            {   GB_cov[3443]++ ;
// covered (3443): 2662428
                                Cx_int64 [pC] = iA + offset ;
                            }
                            else
                            {   GB_cov[3444]++ ;
// covered (3444): 1221396
                                Cx_int32 [pC] = (int32_t) (iA + offset) ;
                            }
                            break ;
                        case GB_FIRSTJ_opcode    : GB_cov[3445]++ ;  
// covered (3445): 3987411
                            // z = first_j(A(iA,jA),y) == jA
                        case GB_FIRSTJ1_opcode   : GB_cov[3446]++ ;  
// covered (3446): 4014850
                            // z = first_j1(A(iA,jA),y) == jA+1
                            if (is64)
                            {   GB_cov[3447]++ ;
// covered (3447): 2673117
                                Cx_int64 [pC] = jA + offset ;
                            }
                            else
                            {   GB_cov[3448]++ ;
// covered (3448): 1381149
                                Cx_int32 [pC] = (int32_t) (jA + offset) ;
                            }
                            break ;
                        case GB_SECONDI_opcode   : GB_cov[3449]++ ;  
// NOT COVERED (3449):
GB_GOTCHA ;
                            // z = second_i(x,B(iB,jB)) == iB
                        case GB_SECONDI1_opcode  : GB_cov[3450]++ ;  
// covered (3450): 3519074
                            // z = second_i1(x,B(iB,jB)) == iB+1
                            if (is64)
                            {   GB_cov[3451]++ ;
// covered (3451): 2022390
                                Cx_int64 [pC] = iB + offset ;
                            }
                            else
                            {   GB_cov[3452]++ ;
// covered (3452): 1533125
                                Cx_int32 [pC] = (int32_t) (iB + offset) ;
                            }
                            break ;
                        case GB_SECONDJ_opcode   : GB_cov[3453]++ ;  
// NOT COVERED (3453):
GB_GOTCHA ;
                            // z = second_j(x,B(iB,jB)) == jB
                        case GB_SECONDJ1_opcode  : GB_cov[3454]++ ;  
// covered (3454): 3240660
                            // z = second_j1(x,B(iB,jB)) == jB+1
                            if (is64)
                            {   GB_cov[3455]++ ;
// covered (3455): 1996574
                                Cx_int64 [pC] = jB + offset ;
                            }
                            else
                            {   GB_cov[3456]++ ;
// covered (3456): 1274181
