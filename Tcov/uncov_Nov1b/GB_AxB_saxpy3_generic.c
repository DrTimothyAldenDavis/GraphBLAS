...
GB_GOTCHA ;
            ...
            switch (opcode)
            {
                case GB_FIRSTI_opcode    : GB_cov[2144]++ ;    // z = first_i(A(i,k),y) == i
// NOT COVERED (2144):
GB_GOTCHA ;
                case GB_FIRSTI1_opcode   : GB_cov[2145]++ ;    // z = first_i1(A(i,k),y) == i+1
// NOT COVERED (2145):
GB_GOTCHA ;
                    #undef  GB_MULT
                    ...
                    break ;
                case GB_FIRSTJ_opcode    : GB_cov[2146]++ ;    // z = first_j(A(i,k),y) == k
// NOT COVERED (2146):
GB_GOTCHA ;
                case GB_FIRSTJ1_opcode   : GB_cov[2147]++ ;    // z = first_j1(A(i,k),y) == k+1
// NOT COVERED (2147):
GB_GOTCHA ;
                case GB_SECONDI_opcode   : GB_cov[2148]++ ;    // z = second_i(x,B(k,j)) == k
// NOT COVERED (2148):
GB_GOTCHA ;
                case GB_SECONDI1_opcode  : GB_cov[2149]++ ;    // z = second_i1(x,B(k,j)) == k+1
// NOT COVERED (2149):
GB_GOTCHA ;
                    #undef  GB_MULT
                    ...
                    #include "GB_AxB_saxpy_template.c"
                    break ;
                case GB_SECONDJ_opcode   : GB_cov[2150]++ ;    // z = second_j(x,B(k,j)) == j
// NOT COVERED (2150):
GB_GOTCHA ;
                case GB_SECONDJ1_opcode  : GB_cov[2151]++ ;    // z = second_j1(x,B(k,j)) == j+1
// NOT COVERED (2151):
GB_GOTCHA ;
                    #undef  GB_MULT
        ...
        else
        {
            ...
            switch (opcode)
            {
                case GB_FIRSTI_opcode    : GB_cov[2152]++ ;    // z = first_i(A(i,k),y) == i
// NOT COVERED (2152):
GB_GOTCHA ;
                case GB_FIRSTI1_opcode   : GB_cov[2153]++ ;    // z = first_i1(A(i,k),y) == i+1
// NOT COVERED (2153):
GB_GOTCHA ;
                    #undef  GB_MULT
                    ...
                    #include "GB_AxB_saxpy_template.c"
                    break ;
                case GB_FIRSTJ_opcode    : GB_cov[2154]++ ;    // z = first_j(A(i,k),y) == k
// NOT COVERED (2154):
GB_GOTCHA ;
                case GB_FIRSTJ1_opcode   : GB_cov[2155]++ ;    // z = first_j1(A(i,k),y) == k+1
// NOT COVERED (2155):
GB_GOTCHA ;
                case GB_SECONDI_opcode   : GB_cov[2156]++ ;    // z = second_i(x,B(k,j)) == k
// NOT COVERED (2156):
GB_GOTCHA ;
                case GB_SECONDI1_opcode  : GB_cov[2157]++ ;    // z = second_i1(x,B(k,j)) == k+1
// NOT COVERED (2157):
GB_GOTCHA ;
                    #undef  GB_MULT
                    ...
                    #include "GB_AxB_saxpy_template.c"
                    break ;
                case GB_SECONDJ_opcode   : GB_cov[2158]++ ;    // z = second_j(x,B(k,j)) == j
// NOT COVERED (2158):
GB_GOTCHA ;
                case GB_SECONDJ1_opcode  : GB_cov[2159]++ ;    // z = second_j1(x,B(k,j)) == j+1
// NOT COVERED (2159):
GB_GOTCHA ;
                    #undef  GB_MULT
