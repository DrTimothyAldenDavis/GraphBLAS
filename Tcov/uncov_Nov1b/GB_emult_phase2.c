...
// covered (3247): 1536
            ...
            if (op->ztype == GrB_INT64)
            {
                switch (opcode)
                {
                    case GB_FIRSTI_opcode     : GB_cov[3248]++ ;  // z = first_i(A(i,j),y) == i
// covered (3248): 96
                    case GB_FIRSTI1_opcode    : GB_cov[3249]++ ;  // z = first_i1(A(i,j),y) == i+1
// covered (3249): 192
                    case GB_SECONDI_opcode    : GB_cov[3250]++ ;  // z = second_i(x,A(i,j)) == i
// covered (3250): 288
                    case GB_SECONDI1_opcode   : GB_cov[3251]++ ;  // z = second_i1(x,A(i,j)) == i+1
// covered (3251): 384
                        #undef  GB_BINOP
                        ...
                        break ;
                    case GB_FIRSTJ_opcode     : GB_cov[3252]++ ;  // z = first_j(A(i,j),y) == j
// covered (3252): 96
                    case GB_FIRSTJ1_opcode    : GB_cov[3253]++ ;  // z = first_j1(A(i,j),y) == j+1
// covered (3253): 192
                    case GB_SECONDJ_opcode    : GB_cov[3254]++ ;  // z = second_j(x,A(i,j)) == j
// covered (3254): 288
                    case GB_SECONDJ1_opcode   : GB_cov[3255]++ ;  // z = second_j1(x,A(i,j)) == j+1
// covered (3255): 384
                        #undef  GB_BINOP
                        #define GB_BINOP(cij, aij, bij, i, j)   \
                            int64_t z = j + offset ;            \
            ...
            else
            {
                switch (opcode)
                {
                    case GB_FIRSTI_opcode     : GB_cov[3256]++ ;  // z = first_i(A(i,j),y) == i
// covered (3256): 96
                    case GB_FIRSTI1_opcode    : GB_cov[3257]++ ;  // z = first_i1(A(i,j),y) == i+1
// covered (3257): 192
                    case GB_SECONDI_opcode    : GB_cov[3258]++ ;  // z = second_i(x,A(i,j)) == i
// covered (3258): 288
                    case GB_SECONDI1_opcode   : GB_cov[3259]++ ;  // z = second_i1(x,A(i,j)) == i+1
// covered (3259): 384
                        #undef  GB_BINOP
                        #define GB_BINOP(cij, aij, bij, i, j)       \
                        ...
                        #include "GB_emult_template.c"
                        break ;
                    case GB_FIRSTJ_opcode     : GB_cov[3260]++ ;  // z = first_j(A(i,j),y) == j
// covered (3260): 96
                    case GB_FIRSTJ1_opcode    : GB_cov[3261]++ ;  // z = first_j1(A(i,j),y) == j+1
// covered (3261): 192
                    case GB_SECONDJ_opcode    : GB_cov[3262]++ ;  // z = second_j(x,A(i,j)) == j
// covered (3262): 288
                    case GB_SECONDJ1_opcode   : GB_cov[3263]++ ;  // z = second_j1(x,A(i,j)) == j+1
// covered (3263): 384
                        #undef  GB_BINOP
                        #define GB_BINOP(cij, aij, bij, i, j)       \
