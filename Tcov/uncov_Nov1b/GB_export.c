...
)
{
    ...
    switch (s)
    {
        case GxB_HYPERSPARSE  : GB_cov[3318]++ ;  
// covered (3318): 3556
            GB_RETURN_IF_NULL (nvec) ;
            GB_RETURN_IF_NULL (Ah) ;

        case GxB_SPARSE  : GB_cov[3319]++ ;  
// covered (3319): 7500
        ...
        case GxB_BITMAP  : GB_cov[3320]++ ;  
// covered (3320): 1536
            GB_RETURN_IF_NULL (nvals) ;
            GB_RETURN_IF_NULL (Ab) ;

        case GxB_FULL  : GB_cov[3321]++ ;  
// covered (3321): 1536
        default: ;
    }
    ...
    switch (s)
    {
        case GxB_HYPERSPARSE  : GB_cov[3322]++ ;  
// covered (3322): 3536
            (*nvec) = (*A)->nvec ;
            (*Ah) = (*A)->h ; (*A)->h = NULL ;

        case GxB_SPARSE  : GB_cov[3323]++ ;  
// covered (3323): 7472
            (*nzmax) = (*A)->nzmax ;
            if (jumbled != NULL)
            {   GB_cov[3324]++ ;
// covered (3324): 7472
        ...
        case GxB_BITMAP  : GB_cov[3325]++ ;  
// covered (3325): 1536
            (*nvals) = (*A)->nvals ;
            (*Ab) = (*A)->b ; (*A)->b = NULL ;

        case GxB_FULL  : GB_cov[3326]++ ;  
// covered (3326): 1536
        default: ;
    }
