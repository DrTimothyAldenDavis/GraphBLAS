...
typedef struct
{
    ...
    int nfine_tasks ;   // # of fine tasks for vector C(:,j)
}
GB_hashtask_struct ;
            ...
            else if (task_flops > 2 * GB_COSTLY * target_task_size)
            {
                ...
                for (int64_t j = j1 ; j < j2 ; j++)
                {
                    ...
                    if (jflops > GB_COSTLY * target_task_size && bjnz > 1)
                    {
                        ...
                        int nfine_tasks = ceil (jflops / target_task_size) ;
                        nfine += nfine_tasks ;
                    }
                }
            ...
            else if (task_flops > 2 * GB_COSTLY * target_task_size)
            {
                ...
                for (int64_t j = j1 ; j < j2 ; j++)
                {
                    ...
                    if (jflops > GB_COSTLY * target_task_size && bjnz > 1)
                    {
                        ...
                        int nfine_tasks = ceil (jflops / target_task_size) ;
                        GB_pslice (&W, Bflops2, bjnz, nfine_tasks) ;
                        ...
                        int master = nf ;
                        for (int fid = 0 ; fid < nfine_tasks ; fid++)
                        {
                            ...
                            TaskList [nf].nfine_tasks = nfine_tasks ;
                            nf++ ;
                        }
                    }
                }
...
//  }
    ...
    // 01, in their 2 least significant bits.  This is used in
    ...
    //==========================================================================
    // numeric phase for coarse tasks
    //==========================================================================
    ...
    // TODO no barrier needed between these two parallel phases

    //==========================================================================
    // final numeric phase for fine tasks
    //==========================================================================
