...
typedef struct
{
    ...
    int64_t *Hi ;       // Hi array for hash table (large coarse hash tasks)
    ...
    int32_t master ;    // master fine task for the vector C(:,j)
    int32_t nfine_team_size ;   // # of fine tasks for vector C(:,j)
    int32_t mark_nbits ;    // # of bits needed for the mark, for a regular
                            // coarse hash task.  Zero for a large coarse
                            // hash task.  Not needed for 1-vector hash task.
    int32_t unused ;
    int64_t mark_bits ;     // a bit mask, with mark_nbits ones in the least
                            // significant part
}
GB_hashtask_struct ;
...
//------------------------------------------------------------------------------
// GB_nbits: # of bits required for an integer
//------------------------------------------------------------------------------

static inline int GB_nbits (int64_t n)
{
    // note that n must be greater than zero
    return ((int) floor (log2 ((double) n) + 1)) ;
}
...
// hash table size, and construct the coarse task.  There are 4 kinds of
// coarse tasks.  Let nj = j2-j1+1 be the # of vectors computed by this task.
// Let mark_nbits = the # of bits required to represent 2*nj+1.
//
// (1) If the hash_size == cnrows, then Gustavson's method is used.
//     Otherwise, the hash method is used (of which there are 3 types, below).
//
// (2)  nj = 1: a single-vector coarse hash task.  Hf can be reused between
//              phase1 and phase2.  mark_nbits is 0 but ignored (a single bit
//              is needed in this case).  Hi is not used.
//
// (3)  nj > 1, mark_nbits > 0:  a regular coarse hash task, where the # of bits
//              required for an index value (cnrows+1), plus the mark_nbits,
//              is 63 or less.  In this case, the Hi workspace is not needed.
//              Hf can represent both the mark value and the index.
//
// (4)  nj > 1, mark_nbits = 0: a large coarse hash task.  The Hi workspace is
//              used for the index, and Hf is used only for the mark.
...
)
{
    ...
    int64_t flmax = 1 ;
    int64_t nj = j2 - j1 + 1 ;      // # of vectors in this coarse task
    int nth = GB_nthreads (nj, chunk, nthreads_max) ;
    ...
    // define the coarse task (1-vector, regular, or large)
    ...
    // determine the # of bits needed for the regular vs large coarse hash task
    int64_t hash_size = TaskList [taskid].hsize ;
    bool use_Gustavson = (hash_size == cnrows) ;
    if (!use_Gustavson && nj > 0)
    {
        int mark_nbits  = GB_nbits (2 * nj + 1) ;
        int index_nbits = GB_nbits (cnrows + 1) ;
        if (mark_nbits + index_nbits < 63)
        {
            // this is a regular multivector coarse hash task
            TaskList [taskid].mark_nbits = mark_nbits ;
            int64_t mark_bits = (((int64_t) 1) << mark_nbits) - 1 ;
            TaskList [taskid].mark_bits = mark_bits ;
        }
    }
}
...
)
{
    ...
    for (int cid = nfine ; cid < ntasks ; cid++)
    {
        ...
        printf (" work %g work/n %g", work, work/cnrows) ;
        printf (" mark_nbits %d\n", TaskList [cid].mark_nbits) ;
    }
    ...
    for (int taskid = 0 ; taskid < ntasks ; taskid++)
    {
        ...
        if (is_fine)
        {
            // 2 kinds of fine tasks, all of them use atomics, and compute
            // A*B(k1:k2,j) for a single vector j.
            if (use_Gustavson)
            {
                nfine_gus++ ;
            }
        ...
        {
            // 4 kinds of coarse tasks.  All of them compute A*B(:,j1:j2).
            // None require atomics.
            ...
            else if (TaskList [taskid].mark_nbits > 0)
            {
                ncoarse_hash++ ;
            }
            else
    ...
    for (int taskid = 0 ; taskid < ntasks ; taskid++)
    {
        ...
        int64_t j2 = TaskList [taskid].end ;
        int64_t nj = j2 - j1 + 1 ;
        int mark_nbits = TaskList [taskid].mark_nbits ;
        ...
        if (!is_fine && !use_Gustavson && nj > 1 && mark_nbits == 0)
        {
            // only large coarse hash tasks need Hi
            printf ("task %d is large coarse hash\n", taskid) ;
...
// #pragma omp flush
        ...
        else
        {
            ...
            else if (nj == 1)
            {
                ...
                // Hf [hash] is zero if the hash entry is empty,
                // or ((i+1) << 1) if it contains entry i.
            ...
            else if (mark_nbits > 0)
            {

                //--------------------------------------------------------------
                // symbolic: regular coarse hash task
                //--------------------------------------------------------------

                // Hf [hash] holds the mark in the lower mark_nbits, and the
                // index i in the upper bits.

                int64_t mark_bits = TaskList [taskid].mark_bits ;
                int64_t hash_bits = (hash_size-1) ;
    ...
    for (int taskid = 0 ; taskid < ntasks ; taskid++)
    {
        ...
        else
        {
            ...
            else if (mark_nbits > 0)
            {
                ...
                //--------------------------------------------------------------
                // hash method for regular coarse task
                //--------------------------------------------------------------
    ...
    printf ("ntasks %d ncoarse %d (gus: %d onehash: %d hash: %d large: %d)"
        " nfine %d (gus: %d hash: %d)\n", ntasks,
        ncoarse, ncoarse_gus, ncoarse_1hash, ncoarse_hash, ncoarse_largehash,
