...
typedef struct
{
    ...
    int64_t unused ;    // to make this struct 64 bytes in size
}
GB_hashtask_struct ;
...
)
{
    ...
    for (int taskid = 0 ; taskid < ntasks ; taskid++)
    {
        ...
        if (taskid != master)
        {
            TaskList [taskid].H = TaskList [master].H ;
//          printf ("slave       %d H %p "GBd"\n", taskid, TaskList [taskid].H,
//              hash_size) ;
        }
        else if (is_fine)
        {
            if (use_Gustavson)
            {
                GB_CALLOC_MEMORY (TaskList [taskid].H, hash_size,
                    sizeof (GB_fine_gus_struct)) ;
//              printf ("fine   gus  %d H %p "GBd"\n",
//                  taskid, TaskList [taskid].H, hash_size) ;
            }
            else
            {
                GB_CALLOC_MEMORY (TaskList [taskid].H, hash_size,
                    sizeof (GB_fine_hash_struct)) ;
//              printf ("fine coarse %d H %p "GBd"\n",
//                  taskid, TaskList [taskid].H, hash_size) ;
            }
        }
        else
        {
            ...
            {
                GB_CALLOC_MEMORY (TaskList [taskid].H, hash_size,
                    sizeof (GB_coarse_gus_struct)) ;
//              printf ("coarse gus  %d H %p "GBd"\n",
//                  taskid, TaskList [taskid].H, hash_size) ;
            }
            else
            {
                GB_CALLOC_MEMORY (TaskList [taskid].H, hash_size,
                    sizeof (GB_coarse_hash_struct)) ;
//              printf ("coarse hash %d H %p "GBd"\n",
//                  taskid, TaskList [taskid].H, hash_size) ;
            }
        }
    }
    ...
    for (int taskid = 0 ; taskid < ntasks ; taskid++)
    {
        ...
        if (is_fine)
        {
            ...
            if (use_Gustavson)
            {
                ...
                GB_fine_gus_struct *GB_RESTRICT H = TaskList [taskid].H ;
            ...
            else
            {
                ...
                GB_fine_hash_struct *GB_RESTRICT H = TaskList [taskid].H ;
...
// #pragma omp flush
        ...
        else
        {
            ...
            if (use_Gustavson)
            {
                ...
                GB_coarse_gus_struct *GB_RESTRICT H = TaskList [taskid].H ;
            ...
            else
            {
                ...
                GB_coarse_hash_struct *GB_RESTRICT H = TaskList [taskid].H ;
...
//              printf ("fine   gus  %d H %p "GBd"\n",
//                  taskid, TaskList [taskid].H, hash_size) ;
                GB_FREE_MEMORY (TaskList [taskid].H, hash_size,
                    sizeof (GB_fine_gus_struct)) ;
            }
            else
            {
