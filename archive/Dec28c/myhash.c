...
)
{
    ...
    int64_t *GB_RESTRICT Fi_all = NULL ;
    double  *GB_RESTRICT Fx_all = NULL ;
...
//    printf ("nthreads %d GB_COSTLY %d\n", nthreads, GB_COSTLY) ;
    ...
    if (nfine > 0)
    {
        GB_MALLOC_MEMORY (Fine_slice, nfine+1, sizeof (int64_t)) ;
    ...
    if (ntasks_initial > 1)
    {
                    ...
                    if (jflops > GB_COSTLY * target_task_size && bjnz > 1)
                    {
                        ...
                        if (jcoarse_start < j)
                        {
                            ...
                            GB_create_coarse_task (jcoarse_start, j-1,
                                TaskList, nc++, Bflops, cnrows, nthreads) ;
                        }
        ...
        GB_FREE_MEMORY (Fine_slice, nfine+1, sizeof (int64_t)) ;
    ...
    // ensure Hi is large enough for the final gather
    Hi_size_total = GB_IMAX (Hi_size_total, nthreads + 1) ;
...
//          printf ("task %d bjnz "GBd" hash_size %g\n", taskid, bjnz,
//              (double) hash_size / (double) cnrows) ;
    ...
    GB_MALLOC_MEMORY (Fi_all, fnz, sizeof (int64_t)) ;
    GB_MALLOC_MEMORY (Fx_all, fnz, sizeof (double)) ;
...
// C(:,j).
    ...
    // GB_FINE_MARK_3RD, in their 2 least significant bits.  This is used in
    ...
    for (int taskid = 0 ; taskid < ntasks ; taskid++)
    {
        ...
        if (is_fine)
        {
            ...
            // TODO prune the task list so there are no empty tasks
            if (fjnz == 0) continue ;
...
//                      if (v != GB_FINE_MARK_3RD) printf ("Hey "GBd"\n",v) ;
                        // C(i,j) += F{taskid}(i)
                        // #pragma omp atomic update
                        Hx [i] += Fx [pF] ;     // atomic MONOID
                        // unlock the critical section for ith entry
                        #pragma omp atomic write
                        Hf [i] = GB_FINE_MARK_3RD ;
                    }

                }
                else
                {
// printf ("num1 gus sparse\n") ;
            ...
            {
// printf ("num1 gus hash\n") ;

                //--------------------------------------------------------------
                // hash method (fine task)
                //--------------------------------------------------------------
                    ...
                    while (1)
                    {
                        ...
                        if (h == i)
                        {
                            ...
                            do
                            {
                                ...
                                // lock, and set those 2 bits to 0.  Otherwise,
                                // another task is in the critical section.
                            } while (v == x) ;
                            // C(i,j) += F{taskid}(i)
                            Hx [hash] += Fx [pF] ;     // atomic MONOID
                            // unlock the critical section for ith entry
                            #pragma omp atomic write
                            Hf [hash] = v ;
#else
        ...
        else
        {
            ...
            if (use_Gustavson)
            {
                ...
                //--------------------------------------------------------------
                // Gustavson's method (coarse task)
                //--------------------------------------------------------------
                ...
                for (int64_t j = j1 ; j <= j2 ; j++)
                {
                    ...
                    else
                    {
                        ...
                        GB_qsort_1a (Ci + Cp [j], cjnz) ;   // coarse
...
//          (double) hash_size / (double) cnrows) ;
        ...
        if (use_Gustavson)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    GB_cumsum (Hi_all, nth, NULL, 1) ;

                    #pragma omp parallel for num_threads(nth) schedule(static)
                    for (int tid = 0 ; tid < nth ; tid++)
                    {
                        int64_t p = pC + Hi_all [tid], istart, iend ;
                        GB_PARTITION (istart, iend, cnrows, tid, nth) ;
                        for (int64_t i = istart ; i < iend ; i++)
                        {
                            if (Hf [i] == GB_FINE_MARK_3RD)
        ...
        else
        {
            ...
            GB_qsort_1a (Ci + Cp [j], cjnz) ;   // fine (do in parallel)
    ...
    printf ("t8: num2 time %g  (%d %d %d)\n", t8, nfine_1, nfine_2, nfine_hash);
    ...
    // mxFree (Fi_all) ;
    // mxFree (Fx_all) ;
    GB_FREE_MEMORY (Fi_all, fnz, sizeof (int64_t)) ;
    GB_FREE_MEMORY (Fx_all, fnz, sizeof (double)) ;
