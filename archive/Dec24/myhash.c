...
)
{
    ...
    if (ntasks_initial > 1)
    {
        ...
        int nc = 0 ;        // coarse task ids are 0:ncoarse-1
        int nf = ncoarse ;  // fine tasks have task id ncoarse:ntasks-1
    ...
    int64_t bnz = Bp [bnvec] ;

    for (int fid = ncoarse ; fid < ntasks ; fid++)
    {
    ...
    int64_t fnz = 0 ;
    for (int taskid = ncoarse ; taskid < ntasks ; taskid++)
    {
...
// C(:,j).
    ...
    for (int taskid = ncoarse ; taskid < ntasks ; taskid++)
    {
        ...
        if (taskid > ncoarse && j == TaskList [taskid-1].vector)
        {
