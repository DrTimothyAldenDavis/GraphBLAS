//------------------------------------------------------------------------------
// myhash3min.c:  C = A*B using a mix of Gustavson's and Hash methods
//------------------------------------------------------------------------------

// Prototype.  Assumes the min-times-double semiring.
...
//                          Hx [i] += t ;           // MONOID
    ...
    // atomic MIN-DOUBLE MONOID
    double old, new, *p = Hx + i ;
    do
    {
        #pragma omp atomic read
        old = (*p) ;
    ...
    while (!__sync_bool_compare_and_swap ((uint64_t *) p,
...
//                              Cx [pC + i] += aik * bkj ;  // MULTIPLY, MONOID
    ...
    // non-atomic MIN-DOUBLE MONOID (and a MULTIPLY)
    Cx [pC + i] = fmin (Cx [pC + i], aik * bkj) ;
