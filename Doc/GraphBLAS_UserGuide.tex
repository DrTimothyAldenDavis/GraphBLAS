...
\title{User Guide for SuiteSparse:GraphBLAS  (TODO:: lots of updates needed)}
...
\item Version 4.0.0 (likely in July, 2020), will follow the V2.0 of the C API.
    {\bf The following is tentative, but all of these changes have been
    implemented in the current draft of v4.0.0 (July 1, 2020).}

    \verb'GrB_wait()', with no inputs is removed.
    \verb'GrB_wait(&object)': polymorphic function is added.
    \verb'GrB_*_nvals' and related functions:
        no longer guarantee completion
        (per the v1.3 C API);
        use \verb'GrB_wait(&object)'
        or non-polymorphic \verb'GrB_*_wait(&object)' instead.

    \verb'GrB_error' has changed; it now takes two parameters,
...
The sparsity status of a matrix can be queried with the following, wihch
...
The \verb'field' parameter can be \verb'GxB_HYPER_SWITCH', \verb'GxB_FORMAT',
...
particular matrix.
The \verb'field' parameter can be \verb'GxB_HYPER_SWITCH' or \verb'GxB_FORMAT'.
...
\verb'field' parameter can be \verb'GxB_HYPER_SWITCH', \verb'GxB_FORMAT'.
\verb'GxB_MODE',
\verb'GxB_NTHREADS', or \verb'GxB_CHUNK'.
For example:
...
\verb'field' parameter can be \verb'GxB_HYPER_SWITCH',
\verb'GxB_SPARSITY_STATUS', or \verb'GxB_FORMAT'.  For example:
...
GraphBLAS can also be combined with user appplications that rely on MPI,
