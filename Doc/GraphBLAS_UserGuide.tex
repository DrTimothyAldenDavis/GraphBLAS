...
An optional environment variable, \verb'GRAPHBLAS_CACHE_PATH', can be used to
control the location of the the cache folder This environment variable is read
by \verb'cmake' when GraphBLAS is compiled, and by \verb'GrB_init' when the
user application runs.  By default it is \verb'~/.SuiteSparse/GraphBLAS/8.0.0'
(for example), where the last three numbers change when the GraphBLAS version
changes.
