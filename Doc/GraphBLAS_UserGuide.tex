...
which by default is \verb'~/.SuiteSparse/GraphBLAS/8.0.0/c' for v8.0.0.  
The kernels are subdivided arbitrarily into subfolders (via an arbitary hash);
otherwise, a single folder may grow too large for efficient access.
Some or all of these kernels can then incorporated into the compiled GraphBLAS
library by copying them into the \verb'GraphBLAS/PreJIT' folder.  Be sure to
move all the \verb'*.c' files into the single \verb'GraphBLAS/PreJIT' folder;
