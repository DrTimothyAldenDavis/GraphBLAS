...
the JIT kernels.  However, compiling GraphBLAS itself requires cmake v3.19 or
later, while compiling the JIT kernels only requires cmake v3.13 or later.
...
\verb'~/.SuiteSparse/GraphBLAS/8.0.0' for GraphBLAS version 8.0.0.  On Windows,
the cache folder is created inside the users' \verb'LOCALAPPDATA' folder,
called \verb'SuiteSparse/GraphBLAS/8.0.0'.  When GraphBLAS starts,
...
which by default is \verb'~/.SuiteSparse/GraphBLAS/8.0.0' for v8.0.0.  The
kernels placed in a subfolder (\verb'c') and inside that folder they are
further subdivided arbitrarily into subfolders (via an arbitary hash).  A
single folder may grow too large for efficient access.  Once GraphBLAS has
generated some kernels, some or all of them kernels can then incorporated into
the compiled GraphBLAS library by copying them into the \verb'GraphBLAS/PreJIT'
folder.  Be sure to move all the \verb'*.c' files into the single
\verb'GraphBLAS/PreJIT' folder; do not keep the subfolder structure.
...
from your cache folder (typically \verb'~/.SuiteSparse/GraphBLAS/8.0.0/c') into
...
applications that use GraphBLAS.  Do not copy the compiled files; they are not
needed and will be ignored.  Just copy the \verb'*.c' files.
