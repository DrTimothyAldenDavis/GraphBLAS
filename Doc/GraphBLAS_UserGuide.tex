...
Unlike the Octave/MATLABinterface (and like the Python interfaces) the Julia 
...
When there is no mask and the mask \verb'COMPLEMENT' option is not selected
the table simplifies (Table~\ref{tab:maskaccum_nomask}).  The
\verb'REPLACE' option no longer has any effect.
The \verb'GrB_SECOND_T' binary operator when used as the accumulator
unifies the first cases, shown in
Table~\ref{tab:maskaccum_nomask_2nd}.  The only difference now is the behavior when
$c_{ij}$ is present but $t_{ij}$ is not.
% With no accumulator at all, $c_{ij}$ is deleted;
% with the \verb'GrB_SECOND_T' operator as the accumulator, $c_{ij}$ remains unchanged.
Finally, the effect of \verb'GrB_FIRST_T' as the accumulator
is shown in Table~\ref{tab:maskaccum_nomask_1st}.
