...
Then \verb's' and \verb'g' are two different sparse scalars that currently have
the same values, but they do not depend on each other.  Modifying one
...
\verb'double'.  This is true for all uses of \verb'GxB_set'.
...
SuiteSparse:GraphBLAS multi-threaded, using only OpenMP.  It is also
thread-safe if it is compiled with OpenMP or POSIX pthreads, and if the user
application threads do not operate on the same matrices at the same time.  The
user threads may use OpenMP or POSIX pthreads.  If multiple user threads make
simultaneous calls to GraphBLAS, then output matrices and vectors used by
different threads must be different, and input matrices and vectors can be
safely used only if any pending computations on them have finished, via
...
\verb'repmat(F,4,4)', if \verb'F' is the \verb'Freescale2' matrix.  With the
vectors \verb'I=randperm(n,5500)' and \verb'J=randperm(n,7000)' and \verb'A' a
random sparse matrix with 38,500 nonzeros, \verb'C(I,J)=A' takes about 30
minutes in MATLAB.\footnote{All performance measurements in this document were
done on a  MacBook Pro, 2.8 GHz Intel Core i7, 16 GB Ram, OSX 10.11.6, clang
8.0.0, MATLAB R2018A.} The same computation takes TODO seconds in
SuiteSparse:GraphBLAS, a speedup of over TODO.  This is after finishing all
pending computations in GraphBLAS and returning result to MATLAB as a valid
MATLAB sparse matrix.  The dominant time complexity for GraphBLAS is $O(n+c)$,
where $n$ is the dimension of \verb'C' and $c$ is its number of nonzeros.  As a
comparison, MATLAB takes just TODO seconds to compute \verb"C+C'" for this
matrix, which also takes time linear in the size of the matrix data structure,
$O(n+c)$.
...
a candidate.  The \verb'maxFirst' semiring uses \verb'z=FIRST(x,y)' as the
