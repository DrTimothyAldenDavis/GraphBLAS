...
\item Version 5.0.0, (April FIXME, 2021)
    ...
    \item (22) bug fix: SIMD vectorization was missing
        ...
        A\&M, with his software package for race-condition detection. Good
        catch!
    \item \verb'GrB_Matrix_reduce_BinaryOp': tagged as historical.
        Use a monoid instead, with
        \verb'GrB_reduce' or \verb'GrB_Matrix_reduce_Monoid'.
    \end{packed_itemize}
...
the latest version of the C API Specification.  The
\verb'GrB_Matrix_reduce_BinaryOp' function was modified in v4.0.1 and limited
to built-in binary operators that correspond to known monoids.  The support of
arbitary binary operators has been deprecated and removed in this version of
SuiteSparse:GraphBLAS.
...
pwereassed in as part of the \verb'thunk', if needed.
\end{alert}
...
%-------------------------------------------------------------------------------
\subsubsection{{\sf GxB\_Matrix\_split:} split matrices   }
%-------------------------------------------------------------------------------
\label{matrix_split}
...
the matrix or vector being constructed have the same uniform value, given by
\verb'Ax[0]' for matrices and \verb'vx[0]' for vectors.  Likewise, on export, a
future release of SuiteSparse:GraphBLAS may return arrays of size large
enough only to hold a single entry.  Even though there may be many more entries
than that in the matrix or vector.  This will be indicated, on both import
and export, with the \verb'is_uniform' boolean flag.

If \verb'is_uniform' is true, then all entries present in the matrix or vector
...
Currently, uniformed-values are not yet supported in this release of
SuiteSparse:GraphBLAS.  On export, \verb'is_uniform' will always be false.
On import, an error is returned if \verb'is_uniform' is true on input.
This feature will be added later, but the \verb'is_uniform' parameter is
added now, so that the API does not need to change once this feature is
implemented.
\end{alert}
...
(
    ...
    bool is_uniform,    // if true, v has uniform values (not yet supported)
...
(
    ...
    bool is_uniform,    // if true, A has uniform values (not yet supported)
...
(assuming it has type \verb'GrB_FP64'):
    ...
    for (int64_t i = 0 ; i < nrows ; i++)
    {
        ...
        for (int64_t p = Ap [i] ; p < Ap [i+1] ; p++)
        {
            ...
            double aij = Ax [is_uniform ? 0 : p] ;   // numerical value
        }
    } \end{verbatim}}
...
%-------------------------------------------------------------------------------
\subsection{{\sf GxB\_BURBLE}, {\sf GxB\_PRINTF}, {\sf GxB\_FLUSH}:
diagnostic output}
%-------------------------------------------------------------------------------
...
To set/get the global options:
    ...
    GxB_get (GxB_FLUSH, void **flush_function) ; \end{verbatim} }
...
of high performance computing in general, but it causes a minor when using this
MATLAB interface for GraphBLAS.  The two use different versions of the same
library, and a segfault arises if the MATLAB interface for v5.x tries to link
with the older GraphBLAS v3.3.3 library.  Likewise, the built-in \verb'C=A*B'
causes a segfault if it tries to use the newer GraphBLAS v4.x or v5.x
libraries.
...
using the folder \newline
\verb'SuiteSparse/GraphBLAS/GraphBLAS/build' instead of in \newline
