...
\verb'GrB_Vector', taking as little as $O(1)$ time.  \verb'GxB_Vector_import'
is identical to
\verb'GxB_Matrix_import_CSC (A, type, n, 1, nvals, &Ap, &i, &x, desc)',
except that the latter creates an \verb'n'-by-1 matrix instead.  For
the vector import, described here , the first argument is a \verb'GrB_Vector',
\verb'vi' and \verb'vx' take the place of \verb'Ai' and \verb'Ax', and the
\verb'Ap' array for the matrix import is not required for a vector import.
...
a \verb'GrB_Vector', taking as little as $O(1)$ time.  \verb'GxB_Vector_export'
is identical to
\verb'GxB_Matrix_export_CSC (&A, &type, &n, &one, &nvals, &Ap, &Ai, &Ax, desc)',
except that the latter exports an \verb'n'-by-1 matrix instead.  For
the vector export, described here, the first argument is a \verb'GrB_Vector',
\verb'vi' and \verb'vx' take the place of \verb'Ai' and \verb'Ax', and the
\verb'Ap' array for the matrix export is not required for a vector export.
...
\begin{spec}
{\bf SPEC:} The \verb'GxB_Matrix_import_CSR'
function is an extension to the spec.
\end{spec}
