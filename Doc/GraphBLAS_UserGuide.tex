...
\vspace{1in}
SuiteSparse:GraphBLAS is under the Apache-2.0 license, except for
the \verb'@GrB' Octave/MATLAB interface, which is licensed under the GNU GPLv3 (or
...
\item Version 5.1.0 (FIXME: date)   % FIXME
    ...
    \item added \verb'GxB_*_pack/unpack' methods as alternatives to  % FIXME
        import/export.
...
the latest version of the C API Specification, and the
\verb'GrB_Matrix_reduce_BinaryOp' function, which is limited to binary
operators that correspond to built-in monoids.
...
\begin{packed_itemize}
    \item If you save a GrB matrix object using one version
        of SuiteSparse:GraphBLAS, you can load it back in in that version or in
        a later version.  You cannot go backwards, and save in v5.x and load in
        v4.x, for example.
    \item \verb'GrB' matrices with dimension larger than \verb'2^53' do not
        display properly in the \verb'whos' command.  Ocatve/MATLAB gets this
        information from \verb'size(A)', which returns a correct result, but
        rounds it to double before displaying it.  The size is displayed
    ...
    \item Non-blocking mode is not exploited; this would require
        ...
        but will be very slow in Octave/MATLAB.
...
passed in as part of the \verb'thunk', if needed.
\end{alert}
...
\verb'GrB_Vector_size'           & size of a vector \\
\verb'GrB_Vector_nvals'          & number of entries in a vector \\
\verb'GxB_Vector_type'           & type of a vector \\
\verb'GrB_Vector_build'          & build a vector from tuples \\
\verb'GxB_Vector_build_Scalar'   & build a vector from tuples \\
...
% new in v5:------------
\verb'GxB_Vector_diag'           & extract a diagonal from a matrix \\
%-----------------------
\verb'GrB_Vector_free'           & free a vector \\
\hline
\hline
\verb'GxB_Vector_import_CSC',    \verb'GxB_Vector_pack_CSC'         & import a vector in CSC format \\
\verb'GxB_Vector_export_CSC',    \verb'GxB_Vector_unpack_CSC'       & export a vector in CSC format \\
\hline
\verb'GxB_Vector_import_Bitmap', \verb'GxB_Vector_pack_Bitmap'      & import a vector in bitmap format \\
\verb'GxB_Vector_export_Bitmap', \verb'GxB_Vector_unpack_Bitmap'    & export a vector in bitmap format \\
\hline
\verb'GxB_Vector_import_Full',   \verb'GxB_Vector_pack_Full'        & import a vector in full format \\
\verb'GxB_Vector_export_Full',   \verb'GxB_Vector_unpack_Full'      & export a vector in full format \\
\hline
\end{tabular}
}
...
\verb'GrB_Matrix_nrows'         & number of rows of a matrix \\
\verb'GrB_Matrix_ncols'         & number of columns of a matrix \\
\verb'GrB_Matrix_nvals'         & number of entries in a matrix \\
\verb'GxB_Matrix_type'          & type of a matrix \\
\verb'GrB_Matrix_build'         & build a matrix from tuples \\
\verb'GxB_Matrix_build_Scalar'  & build a matrix from tuples \\
...
% new in v5:------------
\verb'GxB_Matrix_concat'        & concatenate matrices into a matrix \\
\verb'GxB_Matrix_split'         & split a matrix into matrices \\
\verb'GxB_Matrix_diag'          & diagonal matrix from vector \\
%-----------------------
\verb'GrB_Matrix_free'          & free a matrix \\
\hline
\hline
\verb'GxB_Matrix_import_CSR',      \verb'GxB_Matrix_import_CSR'      & import a matrix in CSR form \\
\verb'GxB_Matrix_export_CSR',      \verb'GxB_Matrix_export_CSR'      & export a matrix in CSR form \\
\hline
\verb'GxB_Matrix_import_CSC',      \verb'GxB_Matrix_import_CSC'      & import a matrix in CSC form \\
\verb'GxB_Matrix_export_CSC',      \verb'GxB_Matrix_export_CSC'      & export a matrix in CSC form \\
\hline
\verb'GxB_Matrix_import_HyperCSR', \verb'GxB_Matrix_import_HyperCSR' & import a matrix in HyperCSR form \\
\verb'GxB_Matrix_export_HyperCSR', \verb'GxB_Matrix_export_HyperCSR' & export a matrix in HyperCSR form \\
\hline
\verb'GxB_Matrix_import_HyperCSC', \verb'GxB_Matrix_import_HyperCSC' & import a matrix in HyperCSC form \\
\verb'GxB_Matrix_export_HyperCSC', \verb'GxB_Matrix_export_HyperCSC' & export a matrix in HyperCSC form \\
\hline
\verb'GxB_Matrix_import_BitmapR',  \verb'GxB_Matrix_import_BitmapR'  & import a matrix in BitmapR form \\
\verb'GxB_Matrix_export_BitmapR',  \verb'GxB_Matrix_export_BitmapR'  & export a matrix in BitmapR form \\
\hline
\verb'GxB_Matrix_import_BitmapC',  \verb'GxB_Matrix_import_BitmapC'  & import a matrix in BitmapC form \\
\verb'GxB_Matrix_export_BitmapC',  \verb'GxB_Matrix_export_BitmapC'  & export a matrix in BitmapC form \\
\hline
\verb'GxB_Matrix_import_FullR',    \verb'GxB_Matrix_import_FullR'    & import a matrix in FullR form \\
\verb'GxB_Matrix_export_FullR',    \verb'GxB_Matrix_export_FullR'    & export a matrix in FullR form \\
\hline
\verb'GxB_Matrix_import_FullC',    \verb'GxB_Matrix_import_FullC'    & import a matrix in FullC form \\
\verb'GxB_Matrix_export_FullC',    \verb'GxB_Matrix_export_FullC'    & export a matrix in FullC form \\
\hline
\end{tabular}
}
\vspace{0.2in}
...
Section~\ref{compare_assign}.  It can be up to 500x faster than MATLAB R2019b,
or even higher depending on the kind of matrix assignment.  MATLAB logical
indexing (the mask of GraphBLAS) is absurdly faster with GraphBLAS as compared
...
\verb'GrB_Matrix_build' and \verb'GrB_Vector_build' can also construct
iso matrices and vectors.  A non-iso matrix/vector is constructed first, and
then the entries are checked to see if they are all equal.

% FIXME: other build: implement post-iso check for GrB*build.
