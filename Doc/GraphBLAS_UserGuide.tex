...
kernels at run time.  See Section~\ref{jit} for details.
...
performance of user-defined types and operators, and also improves the
performance of built-in operators as well.  The JIT can compile kernels that
are specific to the matrix type and the operators that work on it.  In version
v7.4.3 and before, user-defined types and operators were handled by {\em
generic} kernels that used function pointers for each operator and for any
typecasting required.  Even built-in types and operators were sometimes handled
by the generic kernels, if any typecasting was done, or if the specific
operator, monoid, or semiring was disabled by the \verb'Source/GB_control.h'
file when GraphBLAS was compiled.
...
default folder is \verb'~/.SuiteSparse/GraphBLAS/8.0.1' for SuiteSparse:
GraphBLAS version v8.0.1.  The version numbers in the folder name are set
automatically, so that a new version will ignore kernels created by an older
version of GraphBLAS.  If the \verb'GRAPHBLAS_CACHE_PATH' environment variable
is set when GraphBLAS is compiled, that variable defines the folder.

The folder has the following contents, all created by \verb'cmake':

    \begin{itemize}
    \item \verb'GraphBLAS_config.txt': a file that defines the C compiler and
        its flags, and the location of the GraphBLAS source folder.
    \item \verb'cpu': a folder where CPU kernels are created and compiled.
    \item \verb'cuda': a folder where CUDA kernels are created and compiled.
    \end{itemize}
...
To set/get the global options:
    ...
    GxB_set (GxB_C_COMPILER_NAME, const char *compiler) ;
    GxB_get (GxB_C_COMPILER_NAME, const char **compiler) ;
    GxB_set (GxB_C_COMPILER_FLAGS, const char *flags) ;
    GxB_get (GxB_C_COMPILER_FLAGS, const char **flags) ;
    GxB_set (GxB_SOURCE_PATH, const char *src) ;
    GxB_get (GxB_SOURCE_PATH, const char **src) ;
    GxB_set (GxB_CACHE_PATH, const char *cache) ;
    GxB_get (GxB_CACHE_PATH, const char **cache) ;
    \end{verbatim} }
...
\item Version 8.0.0 (FIXME, 2023)

    \begin{itemize}
    \item version 8:  This version is a major SO version increase, since
        it removes a user-visible feature from SuiteSparse:GraphBLAS:
        the \verb'GrB_Descriptor' no longer supports threading control, and some
        features of the \verb'GxB_SelectOp' are removed (see below).
    \item The JIT:  GraphBLAS v8.0.0 now includes a JIT for the CPU kernels,
