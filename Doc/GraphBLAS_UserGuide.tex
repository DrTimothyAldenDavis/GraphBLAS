...
\item Version 3.3.0 (June 25, 2020).  Compliant with V1.3 of the C API
...
The functions \verb'casin' and \verb'casinf' provided by Microsoft
Visual Studio for computing $\sin^{-1}(x)$ when $x$ is complex do not
compute the correct result.  Thus,
\verb'GxB_ASIN_FC32' and \verb'GxB_ASIN_FC64' do not work properly if the MS
...
% TODO in v4.0: GrB_PANIC will not be returned.
...
tests.  The tests in the \verb'Test' folder have been ported to MATLAB on Linux
and MacOS.  The \verb'Test' and \verb'Tcov' tests do not work on Windows.
The MATLAB interface test (\verb'gbtest') does work on all platforms, however.
See the \verb'GraphBLAS/GraphBLAS' folder for more details.
