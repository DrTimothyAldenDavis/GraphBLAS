...
{\bf NOTE: This version of GraphBLAS is a pre-release draft, not yet even
in beta status.  It is fairly stable, but may have a few lurking bugs since
this version has not yet undergone my brutal testing.  I'm making this
widely available to get feedback on the MATLAB interface.  This version
should be just as fast as the latest stable version, V3.0.1, but there might
be a performance bug here and there, so please use V3.0.1 if you would like
to publish performance comparisons.  Use this pre-release version for
...
\item Version 3.1.0 (TODO: DRAFT, 2019).  MATLAB interface added.
    See the \verb'GraphBLAS/GraphBLAS' folder for details and documentation.

\item Version 3.0 (July 26, 2019):
...
The MATLAB interface adds the \verb'gb' class, which is an opaque MATLAB object
that contains a GraphBLAS matrix, either double or single precision, boolean,
or any of the built-in integer types.  Complex matrix support will be added in
the future.    MATLAB sparse and full matrices can be arbitrarily mixed with
GraphBLAS matrices.  The following overloaded operators and methods all work as
you would expect for any matrix.  The matrix multiplication \verb'A*B' uses the
...
{\footnotesize
\begin{verbatim}
abs         double   int64        isreal       minus     prod       sum
all         eig      int8         isscalar     mldivide  rdivide    symamd
amd         end      isa          issparse     mpower    real       symrcm
and         eps      isbanded     issymmetric  mrdivide  repmat     times
any         eq       isdiag       istril       mtimes    round      transpose
bandwidth   find     isempty      istriu       ne        sign       tril
ceil        fix      isfinite     isvector     nnz       single     triu
colamd      floor    isfloat      kron         nonzeros  size       uint16
complex     full     ishermitian  ldivide      norm      sparse     uint32
conj        gb       isinf        le           not       spfun      uint64
ctranspose  ge       isinteger    length       numel     spones     uint8
diag        gt       islogical    logical      nzmax     sqrt       uminus
disp        horzcat  ismatrix     lt           or        subsasgn   uplus
display     int16    isnan        max          plus      subsindex  vertcat
dmperm      int32    isnumeric    min          power     subsref
      \end{verbatim}}

The static methods (used in the form \verb'gb.method') provide direct access
to GraphBLAS functions and can be used on both MATLAB and GraphBLAS matrices.
Type \verb'help graphblas' and \verb'help gb' for more help.

{\footnotesize
\begin{verbatim}
apply      clear           expand         gbkron       reduce        threads
assign     descriptorinfo  extract        gbtranspose  select        type
binopinfo  eadd            extracttuples  monoidinfo   semiringinfo  unopinfo
build      empty           eye            mxm          speye         vreduce
chunk      emult           format         nvals        subassign \end{verbatim}}
