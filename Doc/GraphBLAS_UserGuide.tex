...
\begin{itemize}
    ...
    \item NOTE: if you have JIT kernels in your
        ...
        Its kernels are incompatible with the DRAFT3 version of 8.0.0,
        however.
    ...
    \item {\bf Lots of testing:} some of the new JIT features have not been
        ...
        design of the JIT.  My fully brutal test requires my MATLAB interface
        (see below).
    ...
    The current critical section may be sufficient but this has not been
    tested.

    \item {\bf MATLAB:}
    This version works in MATLAB, where the JIT is disabled by default.
    The MATLAB interface (testing, and the
    \verb'@GrB' interface) does not yet support the JIT.
...
to the matrix type and the operators that work on it.  In version v7.4.3 and
...
v8.0.0 but not because of the JIT.  It must be updated because it uses two
deprecated {\sf GxB\_SelectOp} operators; these are be replaced with {\sf
GrB\_IndexUnaryOp} operators in the {\sf dev} branch of LAGraph.}
...
its flags, and can control when and how the JIT is used.  These changes are
...
\begin{verbatim}
    GxB_set (GxB_JIT_CONTROL, GxB_JIT_OFF) ;
\end{verbatim}
...
The following settings are available for \verb'GxB_JIT_CONTROL'.
...
When GraphBLAS is compiled with \verb'NJIT', the \verb'GxB_JIT_CONTROL' may be
set to \verb'OFF', \verb'PAUSE', or \verb'RUN'.  No kernels will be loaded at
...
\verb'NJIT' option, or by using \verb'GxB_JIT_CONTROL' at run-time, the generic
kernel will be used instead.  This will be slower than the PreJIT or JIT
...
that were specific kernels are only needed for small problems.  In this case,
...
\verb'GrB_init' is called, this body of source code could then be copied into
the user's cache folder.  Not all source is required: just the contents of
...
the \verb'GraphBLAS.h' include file.  GraphBLAS already includes two
compression methods built-in (lz4 and zstd) so this feature would be easy to
add.  All GraphBLAS would need is access to a C compiler, and a cache folder in
which to place its source code and compiled JIT kernels.
...
JIT is in progress.  A releated JIT for SYCL / OpenCL could also be created.

\subsubsection{Creating new user-defined operators from MATLAB, Python, and
Julia} The current methods for creating new operators, monoids, and semirings
...
awkward for users in MATLAB, Python, Julia, and Go.  The \verb'GrB_UnaryOp_new',
\verb'GrB_IndexUnaryOp_new', and \verb'GrB_BnaryOp_new' methods could be
extended to allow those function pointers to be \verb'NULL'.  The JIT kernels
do not require the function pointers at all.  They are used in GraphBLAS in
only a few places, outside of any JIT kernel, when performance is not critical.
For example, they are used when computing single scalars of iso-valued
matrices, and for the accumulate/mask phase after reduction to a scalar.
...
definition, so new user-defined data types originating from MATLAB, Python, or
Julia need no futher work.  All that is needed is a C-style \verb'typedef',
...
JIT to be exploited from MATLAB, Python, Julia, or GoLang applications.
