...
\begin{itemize}
    ...
    \item {\bf working compilers:} This version has been tested on Linux using
    gcc 7.5, 9.4, and 12.2, and clang 10.0, and on the Mac using gcc 12.2 and
    the Apple version of clang.  These compilers all work in the JIT.
    ...
    \item {\bf broken compilers on Linux:}
        icx (2022.0.1 and 2023.0.1) hits a compiler bug with -O3, when
        compiling \newline
        \verb'Source/FactoryKernels/GB_AxB__lxor_pair_bool.c'.
        This occurs when compiling GraphBLAS itself; it has nothing to do
        with the new JIT.
    ...
    \item {\bf MATLAB:}
    This version works in MATLAB, but the MATLAB interface (testing, and the
    \verb'@GrB' interface) does not support the JIT.

    \item {\bf pre-constructed JIT kernels:}  The plan is to allow the
    end user to compile and run GraphBLAS, letting it construct kernels in
    \newline
    \verb'~/.SuiteSparse/GraphBLAS/8.0.0/*.c'.  Next, these \verb'*.c' files
    (or any desired subset) could be copied into
    \verb'GraphBLAS/Source/PreJITKernels', and all of GraphBLAS could then be
    recompiled.   cmake would then incorporate these kernels directly into
    \verb'libgraphblas.so', and the JIT compilation could be disabled in the
    final user application, while at the same time exploiting the JIT for the
    the kernels already compiled.  This feature has not yet been drafted.

    \item {\bf timing metrics:} It would be nice to enable a profile report,
    where each JIT kernel can record the number of times it has been called,
    and the total time spent in each JIT kernel.  This would guide the use
    of pre-constructed JIT kernels.
    ...
    \item \verb'.gitignore' needs to ignore \verb'Source/Shared/GB_config.h',
        which is configured by cmake with the compiler and flags used to
        build GraphBLAS.
...
The CPU JIT can be disabled entirely at compile time by setting the \verb'NJIT'
option \verb'on' in the cmake build options.  The \verb'GxB_JIT_C_CONTROL'
cannot be modified, and \verb'GxB_get (GxB_JIT_C_CONTROL, &c)' will return
\verb'c =' \verb'GxB_JIT_NONE'.  Good performance will be obtained only by
using the \verb'FactoryKernels' that are compiled into GraphBLAS when it is
first compiled with \verb'cmake'.  By default, \verb'NJIT' is off, to enable
the CPU JIT.
...
appear in the \verb'GraphBLAS/Source/FactoryKernels' directory.  If the full
JIT is enabled, GraphBLAS will still be fast, once the JIT kernels are
compiled.  To compile GraphBLAS without its \verb'FactoryKernels', enable the
\verb'COMPACT' option in the cmake build options.  By default, \verb'COMPACT'
is off, to enable the \verb'FactoryKernels'.

If both \verb'NJIT' and \verb'COMPACT' options are enabled, all features of
GraphBLAS will be functional but very slow.  Only generic kernels will be used,
in which every single operator is implemented with a function pointer, and
every scalar assignent requires a \verb'memcpy'.  {\bf This is not recommended,
and the cmake build will issue a warning.  Using these settings to benchmark
GraphBLAS, or for production use in a Linux distro or other widely used
distribution would be inappropriate; do not do it.}
...
\item Version 8.0.0 (March 23, 2023)   % FIXME set the date
    ...
    \item \verb'GxB_SelectOp': GraphBLAS no longer supports
        ...
        as they did in v7.4.3 and earlier.
