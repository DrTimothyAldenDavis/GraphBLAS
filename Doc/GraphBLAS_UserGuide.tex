...
NOTE: this version:
\input{GraphBLAS_version.tex}
is a DRAFT.  The following features/revisions will appear in the stable
v8.0.0 release:
\end{warning}

\begin{itemize}

    \item NOTE: if you have JIT kernels in your
        \verb'~/.SuiteSparse/GraphBLAS/8.0.0' folder, delete them.
        Normally, a new version of GraphBLAS will create a new folder and
        ignore the old one, but this draft is still numbered 8.0.0.
        Its kernels are incompatible with the earlier DRAFT* versions of 8.0.0,
        however.

    \item {\bf Lots of testing:} some of the new JIT features have not been
        fully tested.  Expect to see some bugs.  I am posting this version:

        \input{GraphBLAS_version.tex}

        as a rough draft, not even a beta pre-release, to get feedback on the
        design of the JIT.

    \item {\bf working compilers:} This version has been tested on Linux
    systems (x86-based) using gcc 7.5, 9.4, 12.2, clang 10.0, and icx 2022,
    and on the Mac using gcc 12.2 and the Apple version of clang, and on a
    Power8 system with Linux and gcc 9.4.  These compilers and systems all work
    in the JIT.

    \item {\bf Windows:}
    The JIT needs to be ported to Windows.  This draft will not compile
    on Windows with the JIT enabled.  I think it will compile with \verb'-DNJIT'
    but I haven't tested it.

    \item {\bf compiler warnings:}  using \verb'clang' leads to some
        compiler warnings when compiling the demos (typecasting complex
        types).  These are safe but need to be silenced.

    \item {\bf better performance:}
    ...
    \item {\bf more JIT kernels:} \verb'GrB_assign' (34 kernels), \verb'subref'
    (3 kernels), \verb'masker' (2 kernels), and other (5 kernels).  Grep the
    source code for \verb'JIT: needed' for a list.  The current JIT kernels
    cover all the \verb'generic' kernels in SuiteSparse GraphBLAS v7.x and
    earlier for which {\em factory} kernels are available.  However, in those
    versions, some kernels are only generic.  This draft does not accelerate
    those, but they will be accelerated by the JIT in v8.x.

    \item {\bf FIXME:} Lots of \verb'FIXME''s tagged in the \verb'Source/'
    folder.  These are not broken features, but some will be done for v8.0.0.
    Many of these are optional and should be tagged \verb'FUTURE' instead.

    \item {\bf User-defined operators with NULL function pointers:} With the
        JIT, I could compile kernels for user-defined operators that have no
        function pointer, but just the strings that define the function.
        These kernels would require the JIT to function, and would allow
        systems in other languages such as Python, Julia, and MATLAB to create
        operators at run-time without the need for those systems to pass me a
        pointer to a pre-compiled C function.

    \item {\bf Thread-safety:}  GraphBLAS is thread-safe when used in a single
        multithreaded user application, even when multiple user threads try
        to compile the same JIT kernel at the same time.  However, if the user
        runs multiple instances of GraphBLAS at the same time, from entirely
        different user processed, and each of them attempts to compile the same
        JIT kernels, a file read/write conflict will occur on your
        \verb'~/.SuiteSparse/GraphBLAS/8.0.0' folder.  I need a portable
        file-locking mechanism for Linux/Unix, Mac, and Windows.
        As a short-term workaround, each user application should use its
        own cache folder, instead of sharing a single default cache folder.
...
before, user-defined types and operators were handled by {\em generic} kernels
that used function pointers for each operator and for any typecasting required.
Even built-in types and operators were sometimes handled by the generic
kernels, if any typecasting was done, or if the specific operator, monoid, or
semiring was disabled by the \verb'Source/GB_control.h' file when GraphBLAS was
compiled.
...
folder where it will keep any kernels created and compiled by the JIT.  The
default folder is \verb'~/.SuiteSparse/GraphBLAS/8.0.0' for SuiteSparse:
GraphBLAS version v8.0.0, where the tilde refers to the user's home directory.
...
is not set, then the \verb'cmake' build folder is used instead as a fallback.

The \verb'@GrB' MATLAB interface must compile its JIT kernels slightly
differently, so these are placed in another folder,
\verb'~/.SuiteSparse/GraphBLAS/8.0.0_matlab', by default.  The \verb'*.c'
files are the same but do not mix the compiled kernels.

Two optional environment variables can be used to control the location of the
following two folders.  These environment variables are read by \verb'cmake'
when GraphBLAS is compiled, and by \verb'GrB_init' when the user application
runs:

\newpage
\begin{itemize}
\item \verb'GRAPHBLAS_CACHE_PATH':  this variable gives the path to the
folder where compiled kernels will be kept.  By default it is 
\verb'~/.SuiteSparse/GraphBLAS/8.0.0' (for example), where the last
three numbers change when the GraphBLAS version changes.

\item \verb'GRAPHBLAS_MATLAB_PATH':  this variable gives the path to the cache
folder for compiled kernels for the MATLAB interface,
\verb'~/.SuiteSparse/GraphBLAS/8.0.0_matlab' by default.
It must differ from the \verb'GRAPHBLAS_CACHE_PATH'. % FIXME: check this

\item \verb'GRAPHBLAS_SOURCE_PATH':  this variable gives the path to the
top-level SuiteSparse:GraphBLAS source folder.
\end{itemize}

When the user application starts, it can modify the location of these two
folders after calling \verb'GrB_init'.  It can also modify the C compiler and
its flags, and can control when and how the JIT is used.  This feature is
not available within the MATLAB interface, however.  These changes are
...
\verb'GxB_JIT_SOURCE_PATH'    & \verb'char *' & folder with all of GraphBLAS
                                                 source \\
...
and recompiled.  The stale kernel is overwitten with the new one, so there is
...
and cannot be cleared (see Section~\ref{prejit}).
...
will return \verb'GrB_INVALID_VALUE' to indicate that some kind of JIT failure
occured, or \verb'GrB_OUT_OF_MEMORY' for the last case below.  The error is
also reported if the \verb'GxB_BURBLE' is enabled.

Currently there are no JIT controls made available to the MATLAB interface;
the JIT is always on, unless it has been disabled with the \verb'NJIT'
option.
...
To solve this problem, make sure your application has write permission to
the cache path, the source path contains a valid copy of GraphBLAS,
and that any user-defined types and operators are defined properly so that
no syntax error is detected by the compiler.
...
advance and create all the JIT kernel you need, and then copy them into
...
which by default is \verb'~/.SuiteSparse/GraphBLAS/8.0.0' for v8.0.0.  Some or
all of these kernels can then incorporated into the compiled GraphBLAS library
by copying them into the \verb'GraphBLAS/PreJIT' folder.  If GraphBLAS is then
recompiled via cmake, the build system will detect these kernels, compile them,
and make them available as pre-compiled JIT kernels.  The kernels are no longer
``Just-In-Time'' kernels since they are not compiled at run-time.  They are
refered to as \verb'PreJIT' kernels since they were at one time created at
run-time by the GraphBLAS JIT, but are now compiled into GraphBLAS before it
runs.

{\bf It's that simple.}  Just copy the source files for any kernels you want
from your cache folder (typically \verb'~/.SuiteSparse/GraphBLAS/8.0.0') into
...
applications that use GraphBLAS.  Do not copy the compiled files;
they are not needed and will be ignored.  Just copy the \verb'*.c' files.
...
\verb'gauss' type, and two operators, \verb'addgauss' and \verb'multgauss'.  If
...
generic kernel will be used instead.  This will be slower than the PreJIT or
JIT kernel, but GraphBLAS will still be functional.
...
An identical mechanism is available for the MATLAB interface.  Simply copy
any or all source \verb'*.c' files from your MATLAB cache folder
(\verb'~/.SuiteSparse/GraphBLAS/8.0.0_matlab' by default) into the
\verb'GraphBLAS/GraphBLAS/PreJIT' folder, where the MATLAB interface
resides.  Then recompile \verb'libgraphblas_matlab.so' in that folder.
...
the JIT will compile it if the contorl is set to \verb'GxB_JIT_ON'.
...
\subsubsection{Removing the need for GraphBLAS to find its own source}
The \verb'GxB_JIT_SOURCE_PATH' setting is initialized to the location of the
GraphBLAS source code itself so that new JIT kernels can be compiled that rely
on this source code.  To use the JIT in production, a Linux distro or any other
distro would need to provide the source code of GraphBLAS to the end user
application.  This is a bit awkward and may thus limit the use of the JIT.  A
future GraphBLAS could easily be redesigned where the cmake build system
configures it so that all required source code is ingested into the
\verb'libgraphblas.so' binary compiled library itself.  Then when
\verb'GrB_init' is called, GraphBLAS would extract this body of source code
from its own \verb'libgraphblas.so' binary library, and write it into the
user's cache folder.  Not all source is required: just the contents of
\verb'GraphBLAS/Source/JitKernels', \verb'GraphBLAS/Source/Template',
\verb'GraphBLAS/Source/Shared', and a subset of the \verb'GraphBLAS/Source/*.h'
files.  If compressed, this amounts to just 350KB so the \verb'libgraphblas.so'
file would increase by just this size, and the files would take just 3MB of
space in the user's cache path.  Then by default, any user application or
distro would automatically carry with it the ability to create the source code
for JIT kernels, simply by distributing two files: \verb'libgraphblas.so' and
the \verb'GraphBLAS.h' include file.  GraphBLAS already includes two built-in
compression methods (lz4 and zstd) so this feature would be easy to add.  All
GraphBLAS would need is access to a C compiler and a cache folder in which to
place its source code and compiled JIT kernels.

\subsubsection{SYCL / OpenCL kernels}
...
JIT is in progress.  A related JIT for SYCL / OpenCL could also be created.

\subsubsection{Creating new user-defined operators from MATLAB, Python, Go,
Julia, Java...}
The current methods for creating new operators, monoids, and semirings
requires the user application to pass GraphBLAS a pointer to a compiled C
function.  This is fine for a C/C++ application that uses GraphBLAS, but it is
awkward for users in MATLAB, Python, Julia, Go, Java, etc.  The
\verb'GrB_UnaryOp_new', \verb'GrB_IndexUnaryOp_new', and
\verb'GrB_BinaryOp_new' methods could be extended to allow those function
pointers to be \verb'NULL'.

The JIT kernels do not require the function pointers at all.  They are used in
GraphBLAS in only a few places, outside of any JIT kernel, when performance is
not critical.  For example, they are used when computing single scalars of
iso-valued matrices, and for the accumulate/mask phase after reduction to a
scalar.

For example, consider these excerpts of the \verb'gauss_demo' program in the
...
\begin{verbatim}
    ...
    GxB_BinaryOp_new (&MultGauss, (void *) multgauss,
        Gauss, Gauss, Gauss, "multgauss", MULTGAUSS_DEFN) ;
\end{verbatim} }

GraphBLAS already has everything it needs to create the \verb'gauss' type
definition, so new user-defined data types originating from these other
languages need no futher work.  All that is needed is a C-style \verb'typedef',
provided as a string (such as the \verb'GAUSS_DEFN' string above).

However, GraphBLAS currently requires the \verb'multgauss' function pointer.
If passed a \verb'NULL' function pointer, GraphBLAS could simply compile a copy
of the function itself from the \verb'MULTGAUSS_DEFN' string, and place it in
the user's cache folder.  This feature will make it far easier for the full
JIT to be exploited from applications in languages other than C/C++.
...
typedef enum
{
    ...
    GxB_JIT_C_LINKER_FLAGS = 112,   // CPU JIT C compiler flags
...
\begin{tabular}{ll}
        ...
        \verb'GxB_JIT_SOURCE_PATH'      & folder with all of GraphBLAS source \\
...
To set/get the global options:
    ...
    GxB_set (GxB_JIT_SOURCE_PATH, const char *src) ;
    GxB_get (GxB_JIT_SOURCE_PATH, const char **src) ;
    GxB_set (GxB_JIT_CACHE_PATH, const char *cache) ;
    GxB_get (GxB_JIT_CACHE_PATH, const char **cache) ;
...
\item Version 8.0.0.draft4 (Apr 2, 2023)   % FIXME set the date

    \begin{itemize}
    \item version 8:  This version is a major SO version increase, since
        it removes user-visible features from SuiteSparse:GraphBLAS: the
        \verb'GrB_Descriptor' no longer supports threading control, and some
        features of the \verb'GxB_SelectOp' are removed (see below).
    \item The JIT:  GraphBLAS v8.0.0 includes a JIT for the CPU kernels,
        ...
        options and environment variables to control the JIT.
