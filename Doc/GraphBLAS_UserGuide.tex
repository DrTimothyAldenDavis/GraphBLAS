...
\begin{itemize}
    ...
    \item {\bf Windows: do not attempt.}
    The JIT needs to be ported to Windows.  This draft will not even compile
    on Windows at all, even with the JIT disabled.  This draft uses
    \verb'dlopen', \verb'dlsym', and \verb'dlclose', which do not appear in
    Windows.
    ...
    \verb'@GrB' interface) does not support the JIT.
    ...
    \item {\bf Thread-safety:}  GraphBLAS is thread-safe when used in a single
        multithreaded user application.  However, if the user runs multiple
        instances of GraphBLAS at the same time, and each of them attempts to
        compile the same JIT kernels, a file read/write conflict will occur on
        your \verb'~/.SuiteSparse/GraphBLAS/8.0.0' folder.
...
progress to exploit NVIDIA GPUs.
...
folders.  It can also modify the C compiler and its flags, and can control when
and how the JIT is used.  These changes are made via
\verb'GxB_Global_Option_set', and can be queried via
...
initial setting is \verb'GxB_JIT_ON'.  This can be modified; for example to
...
These settings can be changed by GraphBLAS itself, based on following error
conditions.  These changes affect all kernels, not just the kernel causing the
error.  If any of these cases occur, the call to GraphBLAS will return
\verb'GrB_INVALID_VALUE' to indicate that some kind of JIT failure occured, or
\verb'GrB_OUT_OF_MEMORY' for the last case below.  The error is also reported
if the \verb'GxB_BURBLE' is enabled.

\begin{itemize}

\item When a kernel is loaded that relies on user-defined types and/or
operators, the defintions in the previously compiled kernel are checked against
the current definitions.  If they do not match, the old one is discarded, and a
new kernel will be compiled.  However, if the control is set to
\verb'GxB_JIT_LOAD', no new kernels may be compiled.  To avoid a continual
reloading and checking of kernels, the control is changed from
...
compiled kernel from the cache folder.

\item If an out-of-memory condition occurs in the JIT, the JIT control is
set to \verb'GxB_JIT_PAUSE'.  To solve this, try freeing up memory or solve
smaller problems.

\end{itemize}

In many use cases of GraphBLAS (such as LAGraph), and function will create
a type or operator, use it, and then free it.  It would be far to costly to
clear the loaded kernel and reload it or recompile it each time the
LAGraph function is called, so any kernels that use this type or operator
are kept loaded.  The typical case is that when the LAGraph function is called
again, it will recreate the type or operator with the identical name and
definition.  The kernels that use these types or operators will still be loaded
and can thus be used with no overhead.

However, if a user-defined type or operator is freed and redefined with the
same name but a different definition, any loaded kernels should be freed.  This
case is not detected by GraphBLAS since it would be far too costly to chec
each time a previously loaded kernel is called.  As a result, this condition is
only checked when the kernel is first loaded.  To avoid this issue, if the user
application frees a user-defined type or operator and creates a new one with a
different definition but with the same name, clear all prior kernels by setting
the control to \verb'GxB_JIT_OFF'.  Then turn the JIT back on with
\verb'GxB_JIT_ON'.  The kernels will be checked when reloaded, and recompiled
if their definitions changed.
...
run time (\verb'GxB_JIT_LOAD'), and no new kernels will be compiled at run time
(\verb'GxB_JIT_ON').  Only pre-existing \verb'PreJIT' kernels can be run,
...
\verb'PreJIT' kernels.  Otherwise, generic kernels will be used, in which every
single operator is implemented with a function pointer, and every scalar
assignent requires a \verb'memcpy'.  Generic kernels are slow, so using both
\verb'NJIT' and \verb'COMPACT' options enabled is not recommended when
preparing GraphBLAS for benchmarking, or for a Linux distro or other
widely-used distribution.

%-------------------------------------------------------------------------------
\subsection{Adding {\bf PreJIT} kernels to GraphBLAS}
%-------------------------------------------------------------------------------
...
all of these kernels can then incorporated into the compiled GraphBLAS
library.

JIT kernel source files created by GraphBLAS may be copied into the
\verb'GraphBLAS/PreJIT' folder.  If GraphBLAS is then recompiled via cmake, the
build system will detect these kernels, compile them, and make them available
as pre-compiled JIT kernels.  The kernels are no longer ``Just-In-Time''
kernels since they are not compiled at run-time, but they are still refered to
as JIT kernels since they were at one time created at run time by the GraphBLAS
JIT.  Thus the name \verb'PreJIT'.

If the GraphBLAS library is installed for system-wide usage (say in a Linux
distro, Python, RedisGraph, etc), the \verb'GraphBLAS/PreJIT' kernels will be
available to all users of that library.  They are not disabled by the
\verb'NJIT' option.
...
inside the compiled library itself (\verb'libgraphblas.so' on Linux,
\verb'libgraphblas.dylib' on the Mac).  GraphBLAS will not be any slower if
these kernels are left in the cache folder, and the compiled library size will
not be affected.
...
\verb'GrB_init', and any stale kernels will be ignored.  Likewise, if a
...
Do not delete the \verb'GraphBLAS/PreJIT/GB_prejit.c' index file.  GraphBLAS
uses this to index all the kernels in this \verb'PreJIT' folder, and it is
automatically updated by cmake if new kernels are added.  If you accidentally
delete it, the recompilation will fail.  However, the first failed compilation
will recreate a valid index file, so a second attempt to compile GraphBLAS will
succeed. 

GraphBLAS will detect any stale \verb'PreJIT' kernels and will not use them.
Instead, it will use its runtime JIT to compile new ones with the current
definitions, or it will punt to a generic kernel if JIT compilation is
disabled.  GraphBLAS will be functional, but the stale kernels take up space
inside the compiled GraphBLAS library.  So the best practice is to delete any
stale kernels from the \verb'GraphBLAS/PreJIT' folder, or replace them with
