...
\item Version 4.0.1 ({\bf TODO}, 2020) (beta1)  % TODO:: update when released
...
particular matrix or vector.  The current default value of
\verb'GxB_BITMAP_SWITCH' is 0.10.  If the matrix is currently in bitmap format,
it is converted to full if all entries are present, or to sparse/hypersparse if
$d$ drops below \verb'GxB_BITMAP_SWITCH/2' (5\% by default).  A matrix or
vector with $d$ between 5\% and 10\% (by default) remains in its current
format.
