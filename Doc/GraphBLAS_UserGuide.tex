...
short) is significantly faster than creating matrix with different data values.
A matrix that is iso requires only $O(1)$ space for its numerical values.
The sparse and hypersparse formats require an additional $O(n+e)$ or $O(e)$
integer space to hold the pattern of an $n$-by-$n$ matrix \verb'C', and
a matrix \verb'C' in bitmap format requires $O(n^2)$ space for the bitmap.
A full matrix requires no integer storage, so a matrix that is both iso and
full requires only $O(1)$ space, regardless of its dimension.
...
Consider \verb'GrB_mxm', \verb'GrB_mxv', and \verb'GrB_vxm', and
    ...
    \begin{itemize}
    \item If the the semiring includes a positional multiplicative operator
...
When \verb'C' is iso, the \verb'eWiseMult' of two iso full matrices takes only
$O(1)$ time and memory.
...
When \verb'C' is iso, the \verb'eWiseAdd' of two iso full matrices takes only
$O(1)$ time and memory.
...
operator with \verb's' a scalar.
    ...
    \item If \verb'A' is iso then \verb'C' is iso:

        \begin{itemize}
        \item If the op is \verb'IDENTITY', the $c=a$.
