...
\item
    Open a terminal window and type the following in the
...
\item This generates many files in \verb'SuiteSparse/GraphBLAS/build', and the
    file \verb'graphblas.sln' in particular.  Open the generated
    \verb'graphblas.sln' file in Visual Studio.
...
\item Select the \verb'Build' menu item at the top of the window and
    ...
    This should create a folder called \verb'Release' and place the compiled
    DLL and LIB files there.  Please be patient; some files may take a while to
    compile and sometimes may appear to be stalled.  Just wait.

\item Add the \verb'GraphBLAS/build/Release' folder to the Windows System path.
    (TODO: is this necessary)? % TODO
...
\item As a quick test, try the MATLAB command \verb'GrB(1)', which
    creates a 1-by-1 GraphBLAS matrix.  For a longer test, do the following:
...
\item In Windows, if the tests fail with an error stating that the module
    could not be loaded, it likely means that your Windows System path is
    not set properly, or it is not recognizing the \verb'GraphBLAS/build/Release'
    folder.  Or, you might not have permission to change your Windows System path.
    In this case, do the following in the MATLAB Command Window:
    ...
    need to add the above lines in your \verb'Documents/MATLAB/startup.m'
    file, so that they are done each time MATLAB starts.  You will also need
    to do this after \verb'clear all', since that MATLAB command clears all
    loaded libraries.
