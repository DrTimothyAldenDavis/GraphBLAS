...
SuiteSparse:GraphBLAS conforms to most of Version \input{GraphBLAS_API_version.tex} of
...
\item Version 6.0.0 (future)

    \begin{packed_itemize}
    \item this release will contain only a few changes that cause a
        break with backward compatibility.
    \item It will be fully compliant with the v2.0 C API Specification.
        Two changes from the v2.0 C API Spec are not backward compatible
        (\verb'GrB_*wait', and \verb'GrB_Info').  \verb'GxB_init' has also changed.
        \begin{packed_itemize}
        ...
        \item \verb'GxB_init (mode, malloc, calloc, realloc, free, is_thread_safe)':
            the last parameter, \verb'is_thread_safe', will be deleted.
            The malloc, calloc, realloc, and free functions must be thread-safe.
        \end{packed_itemize}
    \end{packed_itemize}

\item Version 5.2.0 (FIXME: DATE TO BE DETERMINED)
    ...
    \item Added for the v2.0 C API Specification: only features that are
        ...
        \item \verb'GrB_IndexUnaryOp\verb': new, free, fprint, wait
        ...
        \item \verb'GrB_assign', \verb'GrB_subassion': with \verb'GrB_Scalar' input
        ...
        \item \verb'GrB import/export': import/export from/to user-provided arrays
        \item \verb'GrB_EMPTY_OBJECT', \verb'GrB_NOT_IMPLEMENTED': error codes added
        \item \verb'GrB_*_setElement_Scalar': set an entry in a matrix or vector,
            from a \verb'GrB_Scalar'
        \item \verb'GrB_Matrix_diag': same as \verb'GxB_Matrix_diag (C, v, k, NULL)'
    ...
    \item \verb'GxB_Matrix_serialize/deserialize': with compression;
        optional descriptor.
    \item added LZ4/LZ4HC: compression library, \url{http://www.lz4.org} (BSD 2),
        v1.9.3, Copyright (c) 2011-2016, Yann Collet.
    \item (31) bug fix: \verb'C<M>=Z' not returning \verb'C'
...
\verb'GxB_*import', \verb'GxB_*export', \verb'GxB_*pack',
\verb'GxB_*unpack', and \verb'GxB_*serialize' functions described in
Sections \ref{serialize_deserialize} and \ref{import_export},
...
$z=f(a_{ij},i,j,\mbox{thunk})$ that is applied to the entries $a_{ij}$ of an
...
those of user-defined type.  In the table, \verb'k' refers to the \verb'thunk'
...
GraphBLAS name          & Octave/MATLAB     & description \\
                        & analog            & \\
\hline
\verb'GrB_ROWINDEX_INT32'  & \verb'z=i+k'       & row index of \verb'A(i,j)', as int32 \\
\verb'GrB_ROWINDEX_INT64'  & \verb'z=i+k'       & row index of \verb'A(i,j)', as int64 \\
\verb'GrB_COLINDEX_INT32'  & \verb'z=j+k'       & column index of \verb'A(i,j)', as int32 \\
\verb'GrB_COLINDEX_INT64'  & \verb'z=j+k'       & column index of \verb'A(i,j)', as int64 \\
\verb'GrB_DIAGINDEX_INT32' & \verb'z=j-(i+k)'   & column diagonal index of \verb'A(i,j)', as int32 \\
\verb'GrB_DIAGINDEX_INT64' & \verb'z=j-(i+k)'   & column diagonal index of \verb'A(i,j)', as int64 \\
\hline
\verb'GrB_TRIL_INT64'    & \verb'z=(j<=(i+k))'  & true for entries on or below the \verb'k'th diagonal \\
\verb'GrB_TRIU_INT64'    & \verb'z=(j>=(i+k))'  & true for entries on or above the \verb'k'th diagonal \\
\verb'GrB_DIAG_INT64'    & \verb'z=(j==(i+k))'  & true for entries on the \verb'k'th diagonal \\
\verb'GrB_OFFDIAG_INT64' & \verb'z=(j!=(i+k))'  & true for entries not on the \verb'k'th diagonal \\
\verb'GrB_COLLE_INT64'   & \verb'z=(j<=k)'      & true for entries in columns 0 to \verb'k' \\
\verb'GrB_COLGT_INT64'   & \verb'z=(j>k)'       & true for entries in columns \verb'k+1' and above \\
\verb'GrB_ROWLE_INT64'   & \verb'z=(i<=k)'      & true for entries in rows 0 to \verb'k' \\
\verb'GrB_ROWGT_INT64'   & \verb'z=(i>k)'       & true for entries in rows \verb'k+1' and above \\
\hline
\verb'GrB_VALUENE_T'     & \verb'z=(aij!=k)'    & true if \verb'A(i,j)' is not equal to \verb'k'\\
\verb'GrB_VALUEEQ_T'     & \verb'z=(aij==k)'    & true if \verb'A(i,j)' is equal to \verb'k'\\
\verb'GrB_VALUEGT_T'     & \verb'z=(aij>k)'     & true if \verb'A(i,j)' is greater than \verb'k' \\
\verb'GrB_VALUEGE_T'     & \verb'z=(aij>=k)'    & true if \verb'A(i,j)' is greater than or equal to \verb'k' \\
\verb'GrB_VALUELT_T'     & \verb'z=(aij<k)'     & true if \verb'A(i,j)' is less than \verb'k' \\
\verb'GrB_VALUELE_T'     & \verb'z=(aij<=k)'    & true if \verb'A(i,j)' is less than or equal to \verb'k' \\
%
\hline
\end{tabular}
}
...
\verb'GrB_IndexUnaryOp_ttype_name' & return the type of the input {\em thunk} & \ref{idxunop_ttype_name} \\
...
user-defined function $z=f(x,i,j,\mbox{thunk})$, which is used for the
...
user-defined function $z=f(x,i,j,\mbox{thunk})$.  The type may be built-in
or user-defined.
...
operator, which is the type of $z$ in the function $z=f(x,i,j,\mbox{thunk})$.
...
operator, which is the type of $x$ in the function $z=f(x,i,j,\mbox{thunk})$.
This input is used for the entry \verb'A(i,j)' of a matrix or \verb'v(i)' of a
vector.

%-------------------------------------------------------------------------------
\subsubsection{{\sf GxB\_IndexUnaryOp\_ttype\_name:} return the name of the type of {\em thunk}}
%-------------------------------------------------------------------------------
\label{idxunop_ttype_name}

\begin{mdframed}[userdefinedwidth=6in]
{\footnotesize
\begin{verbatim}
GrB_Info GxB_IndexUnaryOp_ttype_name    // return the type_name of the thunk
(
    char *type_name,                    // user array of size GxB_MAX_NAME_LEN
    const GrB_IndexUnaryOp op           // index-unary operator
) ;
...
\verb'GrB_IndexUnaryOp_ttype_name' returns the \verb'ttype' of the index-unary
operator, which is the type of the thunk in the function $z=f(x,i,j,\mbox{thunk})$.
...
Section~\ref{import_export} for import/export/pack/unpack methods.
...
GraphBLAS function   & purpose                                      & Section \\
\hline
\verb'GxB_Matrix_import_CSR',      \verb'GxB_Matrix_pack_CSR'        & import CSR           & \ref{matrix_import_csr}, \ref{matrix_pack_csr} \\
\verb'GxB_Matrix_export_CSR',      \verb'GxB_Matrix_unpack_CSR'      & export CSR           & \ref{matrix_export_csr}, \ref{matrix_unpack_csr} \\
\hline
\verb'GxB_Matrix_import_CSC',      \verb'GxB_Matrix_pack_CSC'        & import CSC           & \ref{matrix_import_csc}, \ref{matrix_pack_csc} \\
\verb'GxB_Matrix_export_CSC',      \verb'GxB_Matrix_unpack_CSC'      & export CSC           & \ref{matrix_export_csc}, \ref{matrix_unpack_csc} \\
\hline
\verb'GxB_Matrix_import_HyperCSR', \verb'GxB_Matrix_pack_HyperCSR'   & import HyperCSR      & \ref{matrix_import_hypercsr}, \ref{matrix_pack_hypercsr} \\
\verb'GxB_Matrix_export_HyperCSR', \verb'GxB_Matrix_unpack_HyperCSR' & export HyperCSR      & \ref{matrix_export_hypercsr}, \ref{matrix_unpack_hypercsr} \\
\hline
\verb'GxB_Matrix_import_HyperCSC', \verb'GxB_Matrix_pack_HyperCSC'   & import HyperCSC      & \ref{matrix_import_hypercsc}, \ref{matrix_pack_hypercsc} \\
\verb'GxB_Matrix_export_HyperCSC', \verb'GxB_Matrix_unpack_HyperCSC' & export HyperCSC      & \ref{matrix_export_hypercsc}, \ref{matrix_unpack_hypercsc} \\
\hline
\verb'GxB_Matrix_import_BitmapR',  \verb'GxB_Matrix_pack_BitmapR'    & import BitmapR       & \ref{matrix_import_bitmapr}, \ref{matrix_pack_bitmapr} \\
\verb'GxB_Matrix_export_BitmapR',  \verb'GxB_Matrix_unpack_BitmapR'  & export BitmapR       & \ref{matrix_export_bitmapr}, \ref{matrix_unpack_bitmapr} \\
\hline
\verb'GxB_Matrix_import_BitmapC',  \verb'GxB_Matrix_pack_BitmapC'    & import BitmapC       & \ref{matrix_import_bitmapc}, \ref{matrix_pack_bitmapc} \\
\verb'GxB_Matrix_export_BitmapC',  \verb'GxB_Matrix_unpack_BitmapC'  & export BitmapC       & \ref{matrix_export_bitmapc}, \ref{matrix_unpack_bitmapc} \\
\hline
\verb'GxB_Matrix_import_FullR',    \verb'GxB_Matrix_pack_FullR'      & import FullR         & \ref{matrix_import_fullr}, \ref{matrix_pack_fullr} \\
\verb'GxB_Matrix_export_FullR',    \verb'GxB_Matrix_unpack_FullR'    & export FullR         & \ref{matrix_export_fullr}, \ref{matrix_unpack_fullr} \\
\hline
\verb'GxB_Matrix_import_FullC',    \verb'GxB_Matrix_pack_FullC'      & import FullC         & \ref{matrix_import_fullc}, \ref{matrix_pack_fullc} \\
\verb'GxB_Matrix_export_FullC',    \verb'GxB_Matrix_unpack_FullC'    & export FullC         & \ref{matrix_export_fullc}, \ref{matrix_unpack_fullc} \\
\hline
...
\newpage
%===============================================================================
\subsection{GraphBLAS import/export/pack/unpack: using move semantics} %========
%===============================================================================
\label{import_export}
...
The semantics of the \verb'GxB' import/export/pack/unpack are the same as the
{\em move constructor} in C++.  On import/pack, the user provides a set of
...
The \verb'GxB' export/unpack of a \verb'GrB_Matrix' or \verb'GrB_Vector' is symmetric with the
...
Exporting/unpacking a matrix or vector forces completion of any pending
...
pending tuples are never exported, but the {\em jumbled} state may be
optionally exported.  In the latter, if the matrix or vector is exported in a
{\em jumbled} state, indices in any row or column may appear out of order.  If
exported as {\em unjumbled}, the indices always appear in ascending order.

The vector import/export/pack/unpack methods use three formats for a
...
If data is imported/packed using the \verb'GxB*_import_*' or
\verb'GxB*_pack_*', the default is to trust the input data so that the
import/pack can be done in $O(1)$ time.  However, if the data comes from an
untrusted source, additional checks should be made during the import.  This is
indicated with a descriptor setting, and then passing the descriptor
to the \verb'GxB' import/pack methods:
...
\verb'GxB_Matrix_export_BitmapC'; see
...
export, the user application is responsible for freeing these
...
exists on output with the type and dimensions unchanged, with no entries.
...
a different strategy as compared to the \verb'GxB' import/export/pack/unpack
methods.  The \verb'GxB' methods are based on {\em move semantics}, in which
ownership of arrays is passed between SuiteSparse:GraphBLAS and the user
application.  This allows import/export/pack/unpack to work in $O(1)$ time, and
require no additional memory, but it requires that GraphBLAS and the user
application agree on which memory manager to use.  This is done via
\verb'GxB_init'.  This allows GraphBLAS to \verb'malloc' an array that can
be later \verb'free'd by the user application, and visa versa.
...
import/export methods, memory is not handed off between the user application
...
for exporting a matrix using \verb'GrB_Matrix_export'.
...
typedef enum
{
    ...
    GxB_IMPORT = 37,        // secure vs fast import/pack
}
GrB_Desc_Field ;
...
\item \verb'GxB_IMPORT' informs the \verb'GxB' import and pack methods
    that they can trust their input data, or not.  The default is to trust
    the input, for faster import.  If the data is being imported from an
...
single scalar is provided as the {\em thunk} input to the index-unary operator.
There are 30 variants, depending on the type of the scalar: (matrix or vector)
x (13 built-in types, one for user-defined types, and a version for
\verb'GrB_Scalar').
...
$z=f(x,i,0,\mbox{thunk})$ to a vector.
The scalar \verb'thunk' can be a non-opaque C scalar corresponding to a built-in
type, a \verb'void *' for user-defined types, or a \verb'GrB_Scalar'.
It is otherwise identical to \verb'GrB_Vector_apply'.
...
$z=f(x,i,j,\mbox{thunk})$ to a matrix.
The scalar \verb'thunk' can be a non-opaque C scalar corresponding to a built-in
type, a \verb'void *' for user-defined types, or a \verb'GrB_Scalar'.
It is otherwise identical to \verb'GrB_Matrix_apply'.
...
type of a {\em thunk} scalar: (matrix or vector) x (13 built-in types,
...
\verb'A' is passed to the \verb'op', as $z=f(a_{ij},i,j,\mbox{thunk})$.  If
...
The \verb'GrB_VALUE*' should not be used with a scalar \verb'thunk' that is
...
\begin{tabular}{lll}
\hline
GraphBLAS name          & Octave/MATLAB     & description (\verb'k' is the value of the {\em thunk}) \\
                        & analog            & \\
\hline
\verb'GrB_ROWINDEX_*'    & \verb'z=i+k'         & select \verb'A(i,j)' if \verb'i != -k' \\
\verb'GrB_COLINDEX_*'    & \verb'z=j+k'         & select \verb'A(i,j)' if \verb'j != -k' \\
\verb'GrB_DIAGINDEX_*'   & \verb'z=j-(i+k)'     & select \verb'A(i,j)' if \verb'j != i+k' \\
\hline
\verb'GrB_TRIL_INT64'    & \verb'z=(j<=(i+k))'  & select entries on or below the \verb'k'th diagonal \\
\verb'GrB_TRIU_INT64'    & \verb'z=(j>=(i+k))'  & select entries on or above the \verb'k'th diagonal \\
\verb'GrB_DIAG_INT64'    & \verb'z=(j==(i+k))'  & select entries on the \verb'k'th diagonal \\
\verb'GrB_OFFDIAG_INT64' & \verb'z=(j!=(i+k))'  & select entries not on the \verb'k'th diagonal \\
\verb'GrB_COLLE_INT64'   & \verb'z=(j<=k)'      & select entries in columns 0 to \verb'k' \\
\verb'GrB_COLGT_INT64'   & \verb'z=(j>k)'       & select entries in columns \verb'k+1' and above \\
\verb'GrB_ROWLE_INT64'   & \verb'z=(i<=k)'      & select entries in rows 0 to \verb'k' \\
\verb'GrB_ROWGT_INT64'   & \verb'z=(i>k)'       & select entries in rows \verb'k+1' and above \\
\hline
\verb'GrB_VALUENE_T'     & \verb'z=(aij!=k)'    & select \verb'A(i,j)' if it is not equal to \verb'k'\\
\verb'GrB_VALUEEQ_T'     & \verb'z=(aij==k)'    & select \verb'A(i,j)' is it equal to \verb'k'\\
\verb'GrB_VALUEGT_T'     & \verb'z=(aij>k)'     & select \verb'A(i,j)' is it greater than \verb'k' \\
\verb'GrB_VALUEGE_T'     & \verb'z=(aij>=k)'    & select \verb'A(i,j)' is it greater than or equal to \verb'k' \\
\verb'GrB_VALUELT_T'     & \verb'z=(aij<k)'     & select \verb'A(i,j)' is it less than \verb'k' \\
\verb'GrB_VALUELE_T'     & \verb'z=(aij<=k)'    & select \verb'A(i,j)' is it less than or equal to \verb'k' \\
%
\hline
\end{tabular}
}
