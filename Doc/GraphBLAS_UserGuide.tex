...
\verb'v = sparse (0)', except that GraphBLAS can create sparse scalars any
...
\verb's' is empty, just as if it were created fresh with \verb'GxB_Sparse_new'.
...
when \verb'GxB_Scalar_nvals' method returns.
%
That is, it acts like an object-specific \verb'GrB_wait' for just this
particular sparse scalar \verb's', which is a side-effect useful in its own
right.  For example, suppose the computations required for \verb's' rely upon a
...
free the variable, \verb'GxB_Scalar_nvals' can be used to force all pending
operations for this sparse scalar \verb's' to complete.  The user application
can then safely modify the global variable.  A call to
\verb'GxB_Scalar_nvals(&nvals,s)' only ensures that the computations require to
compute \verb's' are finished; other pending computations for other objects may
remain.  To ensure that all pending computations are complete for all GraphBLAS
objects, use \verb'GrB_wait' instead.
...
\verb'(double)' \verb'chunk'.  This is necessary if a literal constant
such as \verb'20000' is passed as this argument.  The type of the
constant must be double.  This is true for all uses of \verb'GxB_set'.
