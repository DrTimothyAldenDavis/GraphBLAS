...
\item Version 3.3.0 (TODO NOW, 2020).
    ...
    large floating point values to integers has changed.
    The specific object-based \verb'GrB_Matrix_wait' and related functions
    have been added
    
    The no-argument \verb'GrB_wait( )' has been deprecated.

    Added \verb'GrB_getVersion', \verb'GrB_Matrix_resize',
    \verb'GrB_Vector_resize', \\
    \verb'GrB_kronecker', \verb'GrB_*_wait', ...

% TODO NOW: describe new types, binary/unary ops, monoids, semirings
...
\verb'C=A*B' in 2,291 unique semirings.  With typecasting, any of these 2,291
semirings can be applied to matrices \verb'C', \verb'A', and \verb'B' of any of
the 13 types, in any combination.  This gives $2291 \times 13^3 = 5,033,327$
...
sparse matrix multiplication methods, and often faster when parallelism can be
effectively used.
...
computations for a single object are completed.
...
operations (use \verb'GrB_Matrix_nvals' first).  User
...
created and then calls \verb'GrB_finalize'.
When the user application is finished, exactly
...
built-in types and operators, 2,291 semirings can be built.  This count
...
SuiteSparse:GraphBLAS pre-defines 1,473 of the 2,291 unique semirings that can
be constructed from built-in types and operators, listed below, as an extension
to the spec.  The naming convention is \verb'GxB_add_mult_type', where
\verb'add' is the operator of the additive monoid, \verb'mult' is the multiply
operator, and \verb'type' is the type of inputs to the multiply operator.  The
name of the domain for the additive monoid does not appear, since it always
matches the type of the output of the \verb'mult' operator.  For example
...
\item 54 complex semirings, $T \times T \rightarrow T$ where $T$ is
    ...
    \item 2 complex types, $T$
    \end{itemize}

\item 64 bitwise semirings, $T \times T \rightarrow T$ where $T$ is
    an unsigned integer.
...
\verb'GxB_Scalar_wait'           & wait for pending work on a vector to finish \\
\hline
\end{tabular}
}
...
%-------------------------------------------------------------------------------
\subsubsection{{\sf GxB\_Scalar\_wait:} finish all pending work on a scalar}
%-------------------------------------------------------------------------------
\label{scalar_wait}
...
\verb'GxB_Scalar_wait' forces all pending operations on a scalar to be
completed.
...
\verb'GrB_Vector_wait'           & wait for all pending work to finish \\
\hline
...
%-------------------------------------------------------------------------------
\subsubsection{{\sf GrB\_Vector\_wait:} finish all pending work on a vector}
%-------------------------------------------------------------------------------
\label{vector_wait}
...
\verb'GrB_Vector_wait' forces all pending operations on a vector to be
completed.
...
\verb'GrB_Matrix_wait'          & wait for all pending work to finish \\
\hline
...
%-------------------------------------------------------------------------------
\subsubsection{{\sf GrB\_Matrix\_wait:} finish all pending work on a matrix}
%-------------------------------------------------------------------------------
\label{matrix_wait}
...
\verb'GrB_Matrix_wait' forces all pending operations on a matrix to be
completed.
...
to synchronize user threads, solely for the now-deprected \verb'GrB_wait()'.
...
\hline
\verb'GB_spec_Vector_eWiseMult.m'       & \verb'GrB_Vector_eWiseMult'       & \ref{eWiseMult} \\
\verb'GB_spec_Matrix_eWiseMult.m'       & \verb'GrB_Matrix_eWiseMult'       & \ref{eWiseMult} \\
\verb'GB_spec_Vector_eWiseAdd.m'        & \verb'GrB_Vector_eWiseAdd'        & \ref{eWiseAdd} \\
\verb'GB_spec_Matrix_eWiseAdd.m'        & \verb'GrB_Matrix_eWiseAdd'        & \ref{eWiseAdd} \\
\hline
...
\hline
\verb'GB_spec_kron.m'                   & \verb'GrB_kronecker'              & \ref{kron} \\
\hline
\end{tabular}
}
\vspace{0.2in}
