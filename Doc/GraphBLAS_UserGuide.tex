...
\item Version 5.2.0 (FIXME: DATE TO BE DETERMINED)
    ...
    \item \verb'GxB_eWiseUnion': like \verb'GrB_eWiseAdd' except for how entries in $\bf A\setminus B$
        and $\bf B \setminus A$ are computed.
    ...
    \item performance improvements: \verb'C=A*B': sparse/hyper times bitmap/full,
        and visa versa, including \verb'C += A*B' when \verb'C' is full
    \end{packed_itemize}
...
The \verb'PAIR' operator (also called \verb'ONEB') is simple to describe:
...
\begin{alert}
{\bf SPEC:} \verb'n' may be zero, as an extension to the spec.
\end{alert}
...
\hline
\hline
\verb'GrB_Matrix_import_'$T$    & import in various formats & \ref{GrB_matrix_import} \\
\verb'GrB_Matrix_export_'$T$    & export in various formats & \ref{GrB_matrix_export} \\
...
\begin{alert}
{\bf SPEC:} The spec requires the \verb'type' to always be non-NULL.
...
The \verb'GrB_Matrix_import_'$T$ and \verb'GrB_Matrix_export_'$T$ are not
...
The format for \verb'GrB_Matrix_import_'$T$ and \verb'GrB_Matrix_export_'$T$ is
...
{\footnotesize
\begin{verbatim}
GrB_Info GrB_Matrix_import_TYPE  // import a matrix
(
    GrB_Matrix *A,          // handle of matrix to create
    GrB_Type type,          // type of matrix to create
...
The \verb'GrB_Matrix_import_'$T$ method copies from user-provided arrays into an
opaque \verb'GrB_Matrix' and \verb'GrB_Matrix_export_'$T$ copies data out, from an
...
{\footnotesize
\begin{verbatim}
GrB_Info GrB_Matrix_export_TYPE  // export a matrix
(
...
\verb'GrB_Matrix_export_'$T$ copies the contents of a matrix into three
...
for exporting a matrix using \verb'GrB_Matrix_export_'$T$, using the same
\verb'format'.
...
\verb'GxB_Vector_sort' is identical to the sort of the single column of
an \verb'n'-by-1 matrix.  The descriptor is ignored, except to control
the number of threads to use.  Refer to Section \ref{matrix_sort} for details.
...
\verb'GxB_BY_ROW' format.

The user application can use \verb'GxB_get' and \verb'GxB_set' to set and
