...
\item Version 8.0.0 (TODO, 2023)
    ...
    \item \verb'GrB_Descriptor':
        removed ability to control # of OpenMP threads from the
        descriptor (a rarely used feature).  Replaced with the
        \verb'GxB_Context'
        object.  This change requires a major SO version increase, since
        it removes a user-visible feature from SuiteSparse:GraphBLAS (the
        enum values for the descriptor settings).
    ...
    \item \verb'GxB_SelectOp': GraphBLAS no longer supports
        user-defined GxB_SelectOps.  Use a \verb'GrB_IndexUnaryOp' instead.
        The built-in \verb'GxB_SelectOps' still work, and \verb'GxB_*_select'
        still exists.  However, the \verb'GxB_EQ_THUNK', \verb'GxB_EQ_ZERO',
        \verb'GxB_NE_THUNK', and \verb'GxB_NE_ZERO' operators no longer work on
        user-defined types.  Create a user-defined \verb'GrB_IndexUnaryOp' to
        compute these operations instead, and use \verb'GrB_select'.
