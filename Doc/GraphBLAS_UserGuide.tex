...
\item Version 5.2.0 (FIXME)
    ...
    \item Added for the v2.0 C API Specification: only features that are
        ...
        \item \verb'GrB_select\verb': selection via \verb'GrB_IndexUnaryOp'
    ...
    \item (31) bug fix: \verb'C<M>=Z' not returning \verb'C'
        as iso if \verb'Z 'iso and \verb'C' initialy
...
\verb'GrB_apply'     & apply unary operator    & ${\bf C \langle M \rangle = C \odot} f{\bf (A)}$ \\
                     ...
                     & apply index-unary op    & ${\bf C \langle M \rangle = C \odot} f{\bf (A),i,j,s}$ \\
                     &                         & ${\bf w \langle m \rangle = w \odot} f{\bf (u),i,0,s}$ \\
\hline
...
of the C API Specification, with the exception of h
...
\verb'GrB_IndexUnaryOp'  & a index-unary operator \\
...
\verb'GrB_IndexUnaryOp_free'  & free a user-defined select operator           & \ref{idxunop_free} \\
\hline
\end{tabular}
}
\vspace{0.1in}
...
\verb'GrB_Matrix_exportSize'    & array sizes required for export & \ref{export_size} \\
...
bytes around.  The contents of the array cannot be interpretted except by
...
be later \verb'free'd by the user applicatin, and visa versa.
...
\begin{packed_itemize}
    ...
    \item \verb'GrB_COO_FORMAT': % triplet format (like input to GrB*build)
        Coordinate format.  This is the same format as \verb'GrB_Matrix_build'.
    ...
    \item \verb'GrB_DENSE_COL_FORMAT': % FullC format (GxB_FULL with GxB_BY_ROW)
        ...
        size \verb'nrows*ncols'.
        It holds the matrix in dense format, in colum major order.
\end{packed_itemize}
...
\verb'GrB_apply'     & apply unary operator    & ${\bf C \langle M \rangle = C \odot} f{\bf (A)}$ \\
                     ...
                     &                         & ${\bf w \langle m \rangle = w \odot} f({\bf u},y)$ \\
\hline
\verb'GrB_select'    & apply select operator   & ${\bf C \langle M \rangle = C \odot} f{\bf (A,i,j,k)}$ \\
                     &                         & ${\bf w \langle m \rangle = w \odot} f{\bf (u,i,j,k)}$ \\
\hline
\verb'GrB_reduce'    & reduce to vector        & ${\bf w \langle m \rangle = w \odot} [{\oplus}_j {\bf A}(:,j)]$ \\
                     & reduce to scalar        & $s = s \odot [{\oplus}_{ij}  {\bf A}(I,J)]$ \\
\hline
...
TODO: add \verb'GrB_IndexUnaryOp'
...
suffix, \verb'GxB_Vector_apply_BinaryOp1st' takes as input a \verb'GrB_Scalar'. 
...
operator.  It is otherwise identical to \verb'GrB_Vector_apply'.  With no
suffix, \verb'GxB_Vector_apply_BinaryOp2nd' takes as input a \verb'GrB_Scalar'. 
...
suffix, \verb'GxB_Matrix_apply_BinaryOp1st' takes as input a \verb'GrB_Scalar'. 
...
operator.  It is otherwise identical to \verb'GrB_Matrix_apply'.  With no
suffix, \verb'GxB_Matrix_apply_BinaryOp2nd' takes as input a \verb'GrB_Scalar'. 
To transpose the input matrix, use the \verb'GrB_INP0' descriptor setting.

\newpage
%===============================================================================
\subsection{{\sf GxB\_select:} apply a select operator (FIXME: GrB-select)}
%===============================================================================
\label{select}

TODO: change to \verb'GrB_select' with \verb'GrB_IndexUnaryOp'.
Remove all mention of \verb'GxB_SelectOp'.
...
If \verb'Thunk' is not \verb'NULL', it must be a valid \verb'GrB_Scalar'.
...
which as SuiteSparse:GraphBLAS v1.1.2.  After installing SuiteSparrse:GraphBLAS
...
latest version, while at the same time accessing the v5.0 include file
as \verb'/usr/local/include/GraphBLAS.h'.  This command fails:
