...
%----------------------------------------
\subsubsection{\sf GxB\_JIT\_ERROR\_FALLBACK}
%----------------------------------------

When GraphBLAS attemts to compiler a JIT kernel, it may encounter errors
outside of its control, such as a syntax error in a user-provided operator.  If
the compilation of a kernel or the loading of the compiled kernel fails, the
default is to fallback to a generic kernel instead, since that is always able
to compute the GraphBLAS operation.  If no other errors occur, GraphBLAS
returns \verb'GrB_SUCCESS', even though the JIT failed.  This default action is
suitable for use in production, since it allows the user application to
continue.

However, this default action can make it difficult for a developer to track
down and debug the defintions of user-defined operators.  If the
\verb'GxB_JIT_ERROR_FALLBACK' it set to false, then the fallback does not
occur, and a \verb'GxB_JIT_ERROR' is returned instead.

Repeated JIT failures with the same user-defined kernel would slow down an
application significantly, so ability of the the JIT to load or compile kernels
is removed if a JIT error occurs.  The new setting depends on the kind of error
that occurred:

    \begin{itemize}

    \item For a compiler error, the new setting of \verb'GxB_JIT_C_CONTROL' is
    \verb'GxB_JIT_LOAD'.  No new kernels will be compiled, but previously
    compiled kernels can still be loaded.  Previously loaded kernels can still
    be used.

    \item If a kernel cannot be loaded, or another file I/O error occurs, the
    new setting of \verb'GxB_JIT_C_CONTROL' is \verb'GxB_JIT_RUN'.  No new
    kernels will be compiled, and previously compiled kernels will not be
    loaded.  Previously loaded kernels can still be used.

    \item For other errors (out of memory for example), the new setting of
    \verb'GxB_JIT_C_CONTROL' is \verb'GxB_JIT_PAUSE'.  No new kernels will be
    loaded or compiled, and previously loaded kernels will not be used.

    \end{itemize}

This change to the JIT control setting when an error occurs is not affected by
the \verb'GxB_JIT_ERROR_FALLBACK' setting.
