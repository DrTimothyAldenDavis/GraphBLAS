...
\item Version 6.1.4, Jan 6, 2022 (DRAFT)

    \item \verb'cpu_features':  no longer built as a separate library,
        but built directly into \verb'libgraphblas.so' and
        \verb'libgraphblas.a'.  Added compile-time flags to disable the use of
        \verb'cpu_features' completely.
...
\verb'gcc' compiler, version 4.9 or later is required, but best performance is
obtained in 9.3 or later.  For the Intel \verb'icc' compiler (which is not recommended), version 18.0 or
later is required.  Any version of the Intel \verb'icx' compiler is recommended.  Version
3.13 or later of \verb'cmake' is required; version 3.17 is preferred.

In most cases, \verb'icx' and the Intel OpenMP library result in better
performance than \verb'gcc' and the GNU OpenMP library.
...
\begin{note}
{\bf NOTE:} \verb'icc' is generally an excellent compiler, but it will generate
slower code than \verb'gcc' or \verb'icx' for SuiteSparse:GraphBLAS.  This is
because of how the \verb'icc' compiler treats \verb'#pragma omp atomic'. The
use of \verb'gcc' or \verb'icx' is strongly recommended instead of \verb'icc'.
Atomics are far slower in \verb'icc' as compared to \verb'gcc' and \verb'icx';
the latter two compilers have much faster atomics and work well for
SuiteSparse:GraphBLAS.
\end{note}
...
Using Clang will result in a segfault when you attempt to use the \verb'@GrB'
interface in MATLAB.

\begin{note}
{\bf NOTE:} On the Mac, do not use the Apple compiler (clang, also called cc) 
if you also want to use the MATLAB \verb'@GrB' interface.  GraphBLAS will
work outside of MATLAB, but it will fail with a segfault inside MATLAB
when it attempts to use any OpenMP parallelism.  This limitation
does not apply to using GraphBLAS in MATLAB on Linux, which works with any
compiler.
\end{note}
...
You may encounter a compiler error on the ARM64 architecture when using
the gcc compiler, versions 6.x and earlier.  This error was
encountered on ARM64 Linux with gcc 6.x:
