...
\item Add the \verb'GraphBLAS/build/Release' folder to the Windows System path.
    The \verb'GraphBLAS/Include/GraphBLAS.h' file must be included in user
    applications via \verb'#include "GraphBLAS.h"'
...
\item In Windows, if the tests fail with an error stating that the
    mex file is invalide because the module could not be found, it means
    ...
    recognizing the \verb'GraphBLAS/build/Release' folder.  Or, you might not
    have permission to change your Windows System path.  In this case, do the
