...
\item Version 4.0.0 (Future, 2020), will follow the V2.0 of the C API.
...
\item Version 3.3.0 (June/July (TODO NOW), 2020).  Compliant with V1.3 of the C API
...
Boolean equality, four bitwise operators (AND, OR, XOR, and XNOR),
...
integers), and \verb'NaN' converts to zero.  Positive values outside the range
of the integer are converted to the largest positive integer, and negative
values less than the most negative integer are converted to that most negative
integer.  Other than these special cases, SuiteSparse:GraphBLAS trusts the C
compiler for the rest of its typecasting.
...
See \href{https://github.com/fabianmurariu/graphblas-java-native}.
...
GraphBLAS defines two compile-time constants, \verb'GRB_VERSION' and
\verb'GRB_SUBVERSION', which defines the version of the C API Specification
that is implemented by the library.

If the user program was compiled with one version of the library but linked
with a different one later on, the compile-time version check with
\verb'GRB_VERSION' would be stale.  \verb'GrB_getVersion' thus provides a
runtime access of the version of the C API Specification supported by the
library.
...
Safety is guaranteed if at least one of those conditions is true.
...
{\bf NOTE:} in the upcoming revision of the GraphBLAS C API Specification,
\verb'GrB_error' may change.  It may have the signature
...
\verb'GrB_Type_wait(&type)', except to ensure that it is valid.
...
(
    GrB_UnaryOp *op             // unary operator to wait for
) ;
\end{verbatim}
}\end{mdframed}

After creating a user-defined unary operator, a GraphBLAS library may choose to
exploit non-blocking mode to delay its creation.  \verb'GrB_UnaryOp_wait(&op)'
ensures the \verb'op' is completed.  SuiteSparse:GraphBLAS currently does
nothing for \verb'GrB_UnaryOp_wait(&op)', except to ensure that it is valid.
...
Operators that require the domain to be ordered
(\verb'MIN', \verb'MAX', and relative comparisons less-than, greater-than,
and so on) are not defined for complex types.
...
(
    GrB_BinaryOp *op            // binary operator to wait for
) ;
\end{verbatim}
}\end{mdframed}
...
\verb'GrB_BinaryOp_wait(&op)' ensures the \verb'op' is completed.
SuiteSparse:GraphBLAS currently does nothing for \verb'GrB_BinaryOp_wait(&op)',
except to ensure that it is valid.
...
(
    GxB_SelectOp *op            // select operator to wait for
) ;
\end{verbatim}
}\end{mdframed}
...
to exploit non-blocking mode to delay its creation.
\verb'GxB_SelectOp_wait(&op)' ensures the \verb'op' is completed.
SuiteSparse:GraphBLAS currently does nothing for \verb'GxB_SelectOp_wait(&op)',
except to ensure that it is valid.
...
nothing for \verb'GrB_Monoid_wait(&monoid)', except to ensure that it is valid.
...
exactly with the monoid type.  SuiteSparse:GraphBLAS includes 1,473
...
\item 54 complex semirings, $Z \times Z \rightarrow Z$ where $Z$ is
    \verb'float complex' or \verb'double complex':
...
\verb'GrB_Semiring_wait(&semiring)', except to ensure that it is valid.
...
SuiteSparse:GraphBLAS currently does nothing for
\verb'GrB_Descriptor_wait(&d)', except to ensure that it is valid.
...
descriptor, the presence of an optional binary accumulator operator, the
presence of the optional mask matrix ${\bf M}$, and the status of the mask
...
Consider the case when $c_{ij}$ is present but $t_{ij}$ is not, and there is no
...
present, $c_{ij}$ is not deleted, and keeps its original value.
...
exploits sparsity and does not take $O(n^3)$ time, but what computes is the
following:
...
The \verb'GrB_apply' function is the generic name for 62 specific functions.
...
\item On Windows, the \verb'casin' and \verb'casinf' functions do not
    return the correct imaginary part.  As a result, \verb'GxB_ASIN_FC32'
    and \verb'GxB_ASIN_FC64' do not work properly on Windows.  This
    affects the \verb'GrB/asin' and \verb'GrB/acsc' functions in the
    MATLAB interface.  See the MATLAB tests bypassed in \verb'gbtest76.m'
    for details, in the \verb'GraphBLAS/GraphBLAS/test' folder.
