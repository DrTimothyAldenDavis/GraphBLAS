...
\item Version 3.0 (TODO 2019):  The version number is increased to 3.0, since
...
\verb'A' is passed to the \verb'op', as $z=f(i,j,m,n,a_{ij},\mbox{thunk})$, where \verb'A'
...
For user-defined select operators, if \verb'Thunk' is not \verb'NULL',
then it must be a vector of length 1, with exactly one entry.  The value
of this entry is passed to the user-defined select operator, with no typecasting.
...
As of March 10, 2019, the library includes the algorithms and utilities listed in
