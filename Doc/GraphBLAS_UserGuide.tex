...
\item Version 5.1.0 (TODO)  % FIXME

    \begin{packed_itemize}
    \item iso-valued matrices added
    \end{packed_itemize}
...
and export, with the \verb'iso' boolean flag.
...
SuiteSparse:GraphBLAS.  On export, \verb'iso' will always be false.
On import, an error is returned if \verb'iso' is true on input.
This feature will be added later, but the \verb'iso' parameter is
...
To iterate over the rows and entries of this matrix, the following code can be
used (assuming it has type \verb'GrB_FP64').
Note that if \verb'A' is both full and iso-valued, it takes $O(1)$ memory,
...
four iterations of the \verb'while' loop.  Removing its
