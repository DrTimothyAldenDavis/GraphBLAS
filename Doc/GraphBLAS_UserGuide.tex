...
\begin{spec}
{\bf SPEC:} The \verb'GxB_Vector_import' function is an extension to the spec.
\end{spec}
...
\begin{spec}
{\bf SPEC:} The \verb'GxB_Vector_export' function is an extension to the spec.
\end{spec}
...
The \verb'CSR' format is a matrix with a format (\verb'GxB_FORMAT') of
\verb'GxB_BY_ROW', in standard for instead of hypersparse form (See
...
\begin{itemize}
\item \verb'GrB_Index Ap [nrows+1] ;'  The \verb'Ap' array defines where
the column indices and values appear in \verb'Aj' and \verb'Ax', for 
each row.
...
\verb'sizeof(ctype)', where \verb'ctype' is the exact type in C that correponds
...
The column indices of entries in the ith row of the matrix are held in \verb'Aj
[Ap [i] ... Ap[i+1]]', and the corresponding values are held in the same
positions in \verb'Ax'.  Column indices must be in the range 0 to
...
size at least \verb'nvals'.  If nvals is zero, then the content of the
\verb'Aj' and \verb'Ax' arrays is not accessed and they may be \verb'NULL' on
input (if not \verb'NULL', they are still freed and returned as \verb'NULL').
...
exported is performed, then freeing these three arrays then becomes the
...
\begin{spec}
{\bf SPEC:} The \verb'GxB_Matrix_import_CSC'
function is an extension to the spec.
\end{spec}
...
\begin{spec}
{\bf SPEC:} The \verb'GxB_Matrix_import_HyperCSR'
function is an extension to the spec.
\end{spec}
...
\begin{spec}
{\bf SPEC:} The \verb'GxB_Matrix_import_HyperCSC'
function is an extension to the spec.
\end{spec}
...
\begin{spec}
{\bf SPEC:} The \verb'GxB_Matrix_export_CSR'
function is an extension to the spec.
\end{spec}
...
\begin{spec}
{\bf SPEC:} The \verb'GxB_Matrix_export_CSC'
function is an extension to the spec.
\end{spec}
...
\begin{spec}
{\bf SPEC:} The \verb'GxB_Matrix_export_HyperCSR'
function is an extension to the spec.
\end{spec}
...
\begin{spec}
{\bf SPEC:} The \verb'GxB_Matrix_export_HyperCSC'
function is an extension to the spec.
\end{spec}
