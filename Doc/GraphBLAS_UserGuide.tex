...
\begin{itemize}
    ...
    \item NOTE: if you have JIT kernels in your
        ...
        however.
        Also delete the \verb'~/.SuiteSparse/GraphBLAS/8.0.0/src' folder.
    ...
    \item {\bf FIXME:} Some \verb'FIXME''s tagged in the \verb'Source/'
    folder.
    ...
    \item {\bf Thread-safety:}  GraphBLAS is thread-safe when used in a single
        ...
        different user process, and each of them attempts to compile the same
...
\item {\bf compiler warnings:}

    For the arm-based Mac, the JIT compiler warns about \verb'-arch' every time
    a JIT kernel is compiled inside MATLAB.  These warnings need to be
    silenced.  gcc-12 is compiling x86 code to use inside MATLAB on the
    arm-based Mac, which is required since MATLAB is not a native Arm package
    yet.  The warning is spurious.
...
used.
The optional environment variable, \verb'GRAPHBLAS_CACHE_PATH', is also read by
\verb'GrB_init' when the user application runs.
...
\subsection{Better performance for multithreaded user programs:}
This version is thread-safe when used in a multithread user application, but a
better JIT critical section (many readers, 1 writer) might be needed.  The
...
the performance issues related to this have not been tested.
