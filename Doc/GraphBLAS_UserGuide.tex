...
\item Version 5.0.1 (May 3, 2021)
    ...
    \item \verb'GrB_Matrix_reduce_BinaryOp': tagged as deprecated,
        and to be removed entirely in v6.0.0.  Use a monoid instead, with
...
\verb'GrB_Matrix_reduce_BinaryOp' function.

The \verb'GrB_Matrix_reduce_BinaryOp' function was modified in v4.0.1 and
limited to built-in binary operators that correspond to known monoids.  The
function itself will be removed entirely in SuiteSparse:GraphBLAS v6.0.0.
Reduction in parallel is very difficult without a monoid. 
...
Note that the \verb'is_uniform' feature is not yet implemented.  Once it is
added to SuiteSparse:GraphBLAS, a full matrix with uniform values will take
only $O(1)$ time and memory to create and hold in memory.  Some operations
need not traverse over all entries of the matrix, and these operations will
