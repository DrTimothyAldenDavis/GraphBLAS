...
The size of the type can be returned as an \verb'int32_t' C scalar, or as
a \verb'GrB_Scalar' of type \verb'GrB_UINT64'.  The latter case allows
arbitrary type sizes to be handled.  In practice, SuiteSparse:GraphBLAS
would currently not work with types larger than \verb'INT32_MAX', so either
method will work.  Both of the examples below return the size of
\verb'GrB_FP32' as \verb'sizeof (float)':
...
\begin{alert}
{\bf SPEC:} the v2.1 spec states that the above scalar \verb's' should be of
type \verb'GrB_INT32'.  Such a scalar will work with SuiteSparse:GraphBLAS, but
the type \verb'GrB_UINT64' is recommended, to avoid possible integer overflow
for very large types.
\end{alert}
