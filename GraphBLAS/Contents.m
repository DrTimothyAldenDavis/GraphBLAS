...
%
% The GhB constructor is identical except that it creates a *handle*
% @GhB object, while the @GhB matrix is a MATLAB *value* object.
% A handle object can be modified if it is an input to a MATLAB function,
% while a value object cannot.  The GhB interface can operate on matrices
% in-place, and can leave behind pending work (lazy evaluation) which makes
% it faster to use.  However, the simple MATLAB statement "C=A" differs;
% if A is a @GrB object, C is a copy of A.  If A is a @GhB handle object,
% C is another reference to the underlying object A, and modifying A or C
...
%
% Do not use any method in this folder with the prefix gb*.m or gzb*.m.
% They are internal methods that must be publically visible since they are
% used by both @GrB and @GhB classes.
%
