...
% C = GhB.random (A) has the same pattern as A, but with uniformly
%   distributed random entries.
%
% C = GhB.random (m, n, d) is a random m-by-n GraphBLAS matrix, with
...
% arguments:
%
%   C = GhB.random (..., 'uniform', ...) uses a uniform distribution
%           of values, with entries greater than zero and less than one.
%
%   C = GhB.random (..., 'normal', ...) uses a normal distribution, like
%           the built-in sprandn.
%
%   C = GhB.random (..., 'range', [lo hi], ...) changes the range of
...
%
%   C = GhB.random (A, 'symmetric', ...) creates a symmetric matrix, like
...
%   rng ('default')
%   C = GhB.random (4, 5, 0.5)      % same pattern as A
%   isequal (spones (A), spones (C))
%
%   C = GhB.random (C)              % same pattern but newly random values
%
%   C = GhB.random (2, 4, inf)      % 2-by-4 with all 8 entries
%   C = GhB.random (2, 4, 0.5, 'normal')    % like sprandn (2,4,0.5)
%
%   % random 10-by-10 int16 matrix with entries from -3 to 6,
%   % including explicit zeros, with a uniform distribution
%   C = GhB.random (10, 10, 0.5, 'range', int16 ([-3 6]))
%
%   % larger matrix, with normal distribution:
%   C = GhB.random (1000, 1000, 0.5, 'normal', 'range', int16 ([-3 6])) ;
%   [i,j,x] = find (C) ;
%   histogram (x, 'BinMethod', 'integers') ;
%
%   % lots of explicit zeros, since uint8 saturates:
%   C = GhB.random (1000, 1000, 0.5, 'normal', 'range', uint8 ([-3 6])) ;
%   [i,j,x] = find (C) ;
%   histogram (x, 'BinMethod', 'integers') ;
%
%   % uniform distribution across all possible uint8 values
%   C = GhB.random (1000, 1000, 0.5, 'range', uint8 ([0 255])) ;
%   [i,j,x] = find (C) ;
%   histogram (x, 'BinMethod', 'integers') ;
%
%   % large symmetric matrix with normal distribution
%   C = GhB.random (1e6, 1e-5, 'symmetric', 'normal')
%   [i,j,x] = find (C) ;
%   histogram (x)
%
% See also GhB/sprand, GhB/sprandn, GhB/sprandsym.

% SuiteSparse:GraphBLAS, Timothy A. Davis, (c) 2017-2026, All Rights Reserved.
% SPDX-License-Identifier: Apache-2.0

C = gb_random (1, varargin {:}) ;
