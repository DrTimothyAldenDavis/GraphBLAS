...
% regardless of their value.  The GhB.entries function looks only at the
% pattern of A, not its values.  To exclude explicit entries with a value
% of zero (or any specified additive identity value) use GhB.nonz
% instead.
%
% Let [m n] = size (A)
%
...
%   that I = find (d) is the same I = GhB.entries (A, 'row', 'list').
%
...
%   that I = find (d) is the same I = GhB.entries (A, 'col', 'list').
%
