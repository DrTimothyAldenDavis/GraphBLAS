...
// Use gb.subassign (C, M, scalar) for that case.
...
/*
    ...
    [ai,  ~, ax] = gb.extracttuples (A) ;
    [mi, mj,  ~] = gb.extracttuples (M) ;

    % construct a subset of the entries of the mask M corresponding to the
    % entries in A
    si = mi (ai) ;
    sj = mj (ai) ;
    S = gb.build (si, sj, ax, m, n) ;

    gb.format (save) ;

    % C<M> = S
    C = gb.subassign (C, M, S) ;

*/


// This C mexFunction is faster than the above m-function, since it avoids the
// use of gb.extracttuples.  Instead, it accesses the internal structure of the
