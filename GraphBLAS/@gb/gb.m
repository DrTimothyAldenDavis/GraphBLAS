...
%   format used in MATLAB (see also gb.format), but many graph algorithms are
%   faster if the format is 'by row'.
%
...
%   matrix, or a GraphBLAS sparse matrix object.  G is created as a GraphBLAS
%   sparse matrix object that contains a copy of X, typecasted to the given
%   type if the type string does not match the type of X.  If the type string
%   is not present it defaults to 'double'.
%
% Matrix types:
% 
...
%   that MATLAB supports for its sparse and full matrices is 'by col', that is
%   the default format for GraphBLAS matrices via this MATLAB interfance.
%   However, the default for the C API is 'by row' since graph algorithms tend
%   to be faster with that format.
%
...
%   present), or by the default format (see gb.format).
%
%   When a GraphBLAS matrix is converted into a MATLAB sparse or full matrix,
%   it is always returned to MATLAB 'by col'.
%
% Integer operations:
%
%   Operations on integer values differ from MATLAB.  In MATLAB, uint9(255)+1
%   is 255, since the arithmetic saturates.  This is not possible in matrix
%   operations such as C=A*B, since saturation of integer arithmetic would
%   render most of the monoids useless.  GraphBLAS instead computes a result
%   modulo the word size, so that gb(uint8(255))+1 is zero.  However, new unary
%   and binary operators could be added so that element-wise operations
%   saturate.  The C interface allows for arbitrary creation of user-defined
%   operators, so this could be added in the future.
%
%
...
%       via the descriptor fields desc.in0 and desc.in1, respectively.  To
%       select the format of Cout, use desc.format.  See gb.descriptorinfo for
%       more details.
%
...
methods
    ...
    % Some of these are trivial (like sin and cos, which would be defined for
    % gb matrices of type 'double', 'single', or 'complex').  Others are not
    % appropriate for sparse matrices (such as svd), but the inputs to them
    % could be typecasted to MATLAB full matrices ('double', 'single', or
    % 'complex').  Still more have no matrix inputs (sprand) and thus cannot
    % be overloaded; but gb.sprand would be useful instead as a static method.
    ...
    %    spalloc spaugment spconvert spdiags spparms sprand sprandn sprandsym
    %    sprank spy svds symbfact symmlq symrcm tfqmr treelayout treeplot
    %    unmesh
...
methods (Access = protected, Hidden)
    ...
    %DISP_HELPER display a GraphBLAS matrix
