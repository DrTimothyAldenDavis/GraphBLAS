...
/*
    ...
    gb.format ('by col') ;
    M = gb (m, n, 'logical') ;
    M = gb.select (M, '2nd', 'nonzero', M_input) ;
    if (isequal (gb.format (A), 'by row'))
        A = gb (A) ;
    end
    ...
    G = gb (m, n, gb.type (A)) ;
    G = gb.subassign (G, M, A) ;

    % extract gx = the entries of G
    [~, ~, gx] = gb.extracttuples (G) ;

    % convert G to logical
    G = spones (G, 'logical') ;

    % K = symbolic structure of M, where the kth entry in K(:) is equal to k.
    [mi, mj] = gb.extracttuples (M, struct ('kind', 'zero-based')) ;
    K = gb.build (mi, mj, uint64 (0:mnz-1), m, n) ;

    % T<G> = K
    T = gb (m, n, 'uint64') ;
    T = gb.subassign (T, G, K) ;

    % extract the values from T
    [~, ~, tx] = gb.extracttuples (T) ;

    % construct the result C (always a column vector)
    C = gb.build (tx, zeros(length(gx),1,'uint64'), gx, mnz, 1) ;
...
// use of gb.extracttuples and gb.build.  Instead, it accesses the internal
