    ...
    % See also sparse.
        if (nargin == 1)
            if (isstruct (arg1))
                ...
                % meant for the end-user.  It is only used internally in @GrB.
                % See for example @GrB/mxm, which uses C = GrB (gbmxm (args)),
                % and the typecasting methods, C = double (C), etc.  The output
                % of GrB is a GraphBLAS object.
                C.opaque = arg1 ;
