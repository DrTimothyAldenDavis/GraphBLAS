...
% I = GrB.nonz (A, 'row', 'list')  list of rows with at least one nonzeros
% J = GrB.nonz (A, 'col', 'list')  list of columns with at least one nonzeros
%
...
%   I = find (d) is the same I = GrB.nonz (A, 'row', 'list').
%
...
%   is an implicit zero, not present in the pattern of d, so that
%   I = find (d) is the same I = GrB.nonz (A, 'col', 'list').
%
% With an optional scalar argument as the last argument, the value of the
%   'zero' can be specified; d = GrB.nonz (A, ..., id).  For example, to count
%   all entries in A not equal to one, use GrB.nonz (A, 1)
%
% Example:
%
%   A = magic (5) ;
