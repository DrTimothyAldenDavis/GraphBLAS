...
% assignment, is the same as C = GrB.subassign (C, M, A).  If C and A (or M)
...
% GrB.subassign (C, M, A), so the latter is preferred for best performance.
...
% them to GraphBLAS.  See GrB.subassign instead.  See also the example with
% 'help GrB.extract'.
%
