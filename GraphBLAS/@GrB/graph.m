...
% Graph = graph (G) converts a GraphBLAS matrix G into an undirected Graph.  G
% is assumed to be symmetric; only tril (G) is used by default.  G must be
% square.  If G is logical, then no weights are added to the Graph.  If G is
% single or double, these become the weights of the Graph.  If G is integer,
...
if (omitself)
    ...
    if (isequal (side, 'upper'))
        G2 = gzb_select (1, 'triu', G, 1) ;
    else
        G2 = gzb_select (1, 'tril', G, -1) ;
    end
else
    % include diagonal entries of G
    if (isequal (side, 'upper'))
        G2 = gzb_select (1, 'triu', G, 0) ;
    else
        G2 = gzb_select (1, 'tril', G, 0) ;
    end
end
...
switch (type)
    ...
    case { 'single' }
        ...
        % built-in sparse matrix.  So extract the tuples of G2 first.
        gbmex_wait (G2) ;
    ...
    case { 'logical' }
        ...
        Graph = graph (gbmex_builtin (gzb_cast (1, G2, 'logical')), side) ;

    otherwise

        % typecast to double
        Graph = graph (gbmex_builtin (gzb_cast (1, G2, 'double')), side) ;
