...
(
    ...
    const bool iso,             // if true, allocate A as iso   (NEW)
    GB_Context Context
) ;
...
GrB_Info GB_dup_worker      // make an exact copy of a matrix (NEW: was GB_dup2)
(
    GrB_Matrix *Chandle,    // output matrix, NULL or existing static/dynamic
    const bool C_iso,       // if true, construct C as iso  (NEW)
    const GrB_Matrix A,     // input matrix to copy
...
(
    ...
    const GrB_Index nzmax,  // number of entries the matrix can hold;
                            // ignored if A is iso and full
    const int sparsity,     // sparse (=hyper/auto) / bitmap / full (NEW)
    const bool bitmap_calloc,   // if true, calloc A->b, otherwise use malloc
