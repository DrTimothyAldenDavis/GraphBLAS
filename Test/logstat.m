...
for trial = 1:ntrials
    ...
    if (~isempty (strfind (pwd, 'Tcov')))
        ...
        if (~isempty (GraphBLAS_grbcov))
            ...
            if (c == n)
                fprintf (   '%5d :   all %5d (full 100%% rate: %8.2f/sec)', ...
                    c - clast, n, (c-clast) / t) ;
                fprintf (f, '%5d :   all %5d (full 100%% rate: %8.2f/sec)', ...
                    c - clast, n, (c-clast) / t) ;
            else
                fprintf (   '%5d : %5d of %5d (%5.1f%% rate: %8.2f/sec)', ...
                    c - clast, c, n, 100 * (c/n), (c-clast) / t) ;
                fprintf (f, '%5d : %5d of %5d (%5.1f%% rate: %8.2f/sec)', ...
                    c - clast, c, n, 100 * (c/n), (c-clast) / t) ;
            end
            if (debug)
                fprintf (' [debug]') ;
