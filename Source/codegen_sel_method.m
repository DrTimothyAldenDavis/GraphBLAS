...
fprintf (f, 'm4_define(`GB_iso_select'', `%d'')\n', iso) ;
...
fprintf (f, 'm4_define(`GB_get_zxtypes'', `%s'')\n', zxtype) ;
...
if (isempty (func))
    fprintf (f, 'm4_define(`GB_setup'', `'')\n') ;
    fprintf (f, 'm4_define(`GB_test_value_of_entry'', `(no test; %s ignores values)'')\n', opname) ;
elseif (iscell (func))
    fprintf (f, 'm4_define(`GB_setup'', `%s'')\n', func {1}) ;
...
if (is_entry_selector || is_nonzombie_selector)
    fprintf (f, 'm4_define(`GB_kind'', `#define GB_ENTRY_SELECTOR'')\n') ;
else
    fprintf (f, 'm4_define(`GB_kind'', `#define %s'')\n', kind) ;
end

% get scalar thunk
if (~isequal (atype, 'GB_void') && ~isempty (strfind (opname, 'thunk')))
    fprintf (f, 'm4_define(`GB_get_thunk'', `%s thunk = (*athunk) ;'')\n', atype) ;
else
    fprintf (f, 'm4_define(`GB_get_thunk'', `'')\n') ;
end
...
if (iso)
    % A is iso
    fprintf (f, 'm4_define(`GB_select_entry'', `/* assignment skipped, C and A are iso */'')\n') ;
elseif (isequal (opname, 'eq_zero'))
    % create C as iso for all EQ_ZERO ops even when A is not iso, with iso value zero
    fprintf (f, 'm4_define(`GB_select_entry'', `/* assignment skipped, C is iso with all entries zero */'')\n') ;
...
elseif (isequal (opname, 'nonzero') && isequal (atype, 'bool'))
    ...
    fprintf (f, 'm4_define(`GB_select_entry'', `/* assignment skipped, C is iso with all entries true */'')\n') ;
elseif (isequal (atype, 'GB_void'))
    fprintf (f, 'm4_define(`GB_select_entry'', `memcpy (Cx +((pC)*asize), Ax +((pA)*asize), asize)'')\n') ;
else
...
cmd = sprintf ('cat control.m4 Generator/GB_sel.c | m4 -P | awk -f codegen_blank.awk > Generated1/GB_sel__%s.c', name) ;
fprintf ('.') ;
system (cmd) ;
