    ...
    { 
        ...
        if (suffix == NULL)
        {
            snprintf (kernel_name, KLEN-1,
                "GB_jit_AxB_saxbit_%0*" PRIx64, 16, scode) ;
        }
        else
        {
            snprintf (kernel_name, KLEN-1,
                "GB_jit_AxB_saxbit_%0*" PRIx64 "__%s", 16, scode, suffix) ;
        }
...
//      printf ("result: %d\n", res) ;
        ...
        if (need_to_compile)
        {
            ...
            fprintf (fp, "\n#include \"GB_jit_kernel_AxB_saxbit.c\"\n") ;
    ...
    GrB_Info info = GB_jit_kernel (C, M, A, B, ntasks, nthreads,
