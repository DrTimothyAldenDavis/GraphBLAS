...
)
{
    ...
    uint64_t hash = GB_encodify_ewise (&encoding, &suffix,
        GB_JIT_KERNEL_EMULT4, true, false, false, false, false,
        C_sparsity, C->type, M, Mask_struct, false, binaryop,
        false, A, B) ;
    if (hash == UINT64_MAX)
    {
    ...
    { 
        ...
        if (suffix == NULL)
        {
            snprintf (kernel_name, KLEN-1,
                "GB_jit_emult_04_%0*" PRIx64, 13, scode) ;
        }
        else
        {
            snprintf (kernel_name, KLEN-1,
                "GB_jit_emult_04_%0*" PRIx64 "__%s", 13, scode, suffix) ;
        }
...
//      printf ("result: %d\n", res) ;
        ...
        if (need_to_compile)
        {
            ...
            fprintf (fp, "\n#include \"GB_jit_kernel_emult_04.c\"\n") ;
    ...
    GrB_Info info = GB_jit_kernel (C, M, Mask_struct, A, B,
