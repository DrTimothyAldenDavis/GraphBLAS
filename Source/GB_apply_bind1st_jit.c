...
)
{
    ...
    uint64_t hash = GB_encodify_ewise (&encoding, &suffix,
        GB_JIT_KERNEL_APPLYBIND1, false, false, false, false, false,
    ...
    { 
        ...
        if (suffix == NULL)
        {
            snprintf (kernel_name, KLEN-1,
                "GB_jit_apply_bind1st_%0*" PRIx64, 13, scode) ;
        }
        else
        {
            snprintf (kernel_name, KLEN-1,
                "GB_jit_apply_bind1st_%0*" PRIx64 "__%s", 13, scode, suffix) ;
        }
...
//      printf ("result: %d\n", res) ;
        ...
        if (need_to_compile)
        {
            ...
            GB_macrofy_ewise (fp, scode, binaryop, ctype, NULL, B->type) ;
            fprintf (fp, "\n#include \"GB_jit_kernel_apply_bind1st.c\"\n") ;
    ...
    GrB_Info info = GB_jit_kernel (Cx, xscalar, B->x, B->b,
