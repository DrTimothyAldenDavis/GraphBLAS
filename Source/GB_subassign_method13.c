...
)
{
    ...
    // PARALLEL: split S,M,A into coarse/fine tasks.  This is like the parallel
    // masked add, C<M>=A+B, but !M is also handled.  Use GB_ewise_slice. (S is
    // B, A is A, M is M).  No need to slice C.
