...
)
{
    ...
    if (newsize > Pending->nmax)
    {
        ...
        Pending->i = GB_REALLOC (Pending->i, newsize, Pending->nmax,
            int64_t, &ok1) ;
        if (Pending->j != NULL)
        { 
            Pending->j = GB_REALLOC (Pending->j, newsize, Pending->nmax,
                int64_t, &ok2) ;
        }
        size_t s = Pending->size ;
        Pending->x = GB_REALLOC (Pending->x, newsize*s, (Pending->nmax)*s,
            GB_void, &ok3) ;
