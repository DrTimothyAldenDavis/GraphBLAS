...

{
    ...
    // FIXME: for the JIT, break this into 3 kernels:
    if (use_coarse_tasks)
    {
        ...
        for (tid = 0 ; tid < ntasks ; tid++)
        {
            ...
            for (int64_t kk = kfirst ; kk < klast ; kk++)
            {
                ...
                for ( ; pA < pA_end ; pA++)
                {
                    ...
                    // FIXME: if the monoid is not atomic, create an int8
                    // array, Hf, the same size as C, initially zero.
                    // Then use this to lock:
                    // do { GB_ATOMIC_CAPTURE_INT8 (f, Hf [i,j], 1) ; }
                    // while (f == 1) ;
                    // and to unlock:
                    // GB_ATOMIC_WRITE
                    // Hf [i,j] = 0 ;
