...
{ 
    ...
    for (int64_t outer = 0 ; outer < csc ? coltiles : rowtiles ; outer++)
    {
        for (int64_t inner = 0 ; inner < csc ? rowtiles : coltiles ; inner++)
        {
            ...
            const int64_t *GB_RESTRICT kfirst_Aslice = NULL ;
            const int64_t *GB_RESTRICT klast_Aslice  = NULL ;
            const int64_t *GB_RESTRICT pstart_Aslice = NULL ;

            if (sparsity == GxB_FULL || sparsity == GxB_BITMAP)
            {
                // the tile matrix A is full or bitmap
                A_nthreads = GB_nthreads (anz, chunk, nthreads_max) ;
            }
            else
            {
                // the tile matrix A is sparse or hypersparse
                GB_SLICE_MATRIX (A, 1, chunk) ;
                kfirst_Aslice = A_ek_slicing ;
                klast_Aslice  = A_ek_slicing + A_ntasks ;
                pstart_Aslice = A_ek_slicing + A_ntasks * 2 ;
            }
