//------------------------------------------------------------------------------
// GxB_rmm_init: initialize GraphBLAS with RMM
//------------------------------------------------------------------------------
...
// GxB_rmm_init is identical to GrB_init, except that it allows the user
// application to use or create an RMM memory pool.  the SuiteSparse:GraphBLAS
// will use.
...
/*
    ...
    // to use RMM, without CUDA

        #include "GraphBLAS.h"
        GxB_rmm_init (mode, NULL) ;     // let GraphBLAS initialize RMM
        GxB_rmm_init (mode, rmm_pool) ; // give GraphBLAS a prior RMM pool to use
        GxB_rmm_malloc
        GxB_rmm_free
        GxB_Global_Option_get (GxB_RMM_POOL, &rmm_pool) ;

    // to use CUDA (and also RMM), do not use this function.  Instead use the
    // following:
