    ...
    { 
        ...
        if (suffix == NULL)
        {
            snprintf (kernel_name, KLEN-1,
                "GB_jit_apply_bind2nd_%0*" PRIx64, 13, scode) ;
        }
        else
        {
            snprintf (kernel_name, KLEN-1,
                "GB_jit_apply_bind2nd_%0*" PRIx64 "__%s", 13, scode, suffix) ;
        }
...
//      printf ("result: %d\n", res) ;
        ...
        if (need_to_compile)
        {
            ...
            GB_macrofy_ewise (fp, scode, binaryop, ctype, A->type, NULL) ;
            fprintf (fp, "\n#include \"GB_jit_kernel_apply_bind2nd.c\"\n") ;
    ...
    GrB_Info info = GB_jit_kernel (Cx, A->x, yscalar, A->b,
