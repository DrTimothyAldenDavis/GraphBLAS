    ...
    { 
        ...
        if (suffix == NULL)
        {
            snprintf (kernel_name, KLEN-1,
                "GB_jit_trans_bind1st_%0*" PRIx64, 13, scode) ;
        }
        else
        {
            snprintf (kernel_name, KLEN-1,
                "GB_jit_trans_bind1st_%0*" PRIx64 "__%s", 13, scode, suffix) ;
        }
...
//      printf ("result: %d\n", res) ;
        ...
        if (!need_to_compile && !builtin)
        {
            ...
            need_to_compile =
                ...
                !GB_jitifyer_match_defn (dl_query, 4, A->type->defn) ;
            if (need_to_compile)
            {
        ...
        if (need_to_compile)
        {
            ...
            GB_macrofy_ewise (fp, scode, binaryop, C->type, NULL, A->type) ;
            fprintf (fp, "\n#include \"GB_jit_kernel_trans_bind1st.c\"\n") ;
    ...
    GrB_Info info = GB_jit_kernel (C, xscalar, A, Workspaces, A_slice,
