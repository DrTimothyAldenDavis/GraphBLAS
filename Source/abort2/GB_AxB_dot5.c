...
)
{
    ...
    // mask present, complemented, valued, full+valued or bitmap+structural, and GrB_INT32
                ...
                if (!mij)
                {
                    ...
                    if (ainz > 8)
                    {
                        ...
                        for ( ; pA < pA_end - 8 ; pA += 8)
                        {
                            ...
                            for (int kk = 0 ; kk < 8 ; kk++)
                            {
                                Bb_panel [kk] = Bb [pB_start + Ai [pA + kk]] ;
                                b_exists = b_exists | Bb_panel [kk] ;
                            }
                            // Z += A (panel,i) .* B (panel,j)
                            if (b_exists)
                            {
                                #if GB_IS_ANY_MONOID
                                for (int kk = 0 ; kk < 8 ; kk++)
                                ...
                                for (int kk = 0 ; kk < 8 ; kk++)
                                {
                                    ...
                                    if (b)
                                    {
                                        GB_GETA (aki, Ax, pA) ;
                                        GB_GETB (bkj, Bx, pB_start+k) ;
                                        GB_MULTADD (Zx [kk], aki, bkj, i, k, j);
                                    }
                                }
                                #endif
                            }

                            // TODO: if (# of panels % 16) check terminal
                        }
