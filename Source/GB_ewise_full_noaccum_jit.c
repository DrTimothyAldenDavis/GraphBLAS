...
)
{
    ...
    uint64_t hash = GB_encodify_ewise (&encoding, &suffix,
        GB_JIT_KERNEL_EWISEFN, false, false, false, false, false,
    ...
    { 
        ...
        if (suffix == NULL)
        {
            snprintf (kernel_name, KLEN-1,
                "GB_jit_ewise_full_noaccum_%0*" PRIx64, 13, scode) ;
        }
        else
        {
            snprintf (kernel_name, KLEN-1,
                "GB_jit_ewise_full_noaccum_%0*" PRIx64 "__%s", 13, scode, suffix) ;
        }
...
//      printf ("result: %d\n", res) ;
        ...
        if (need_to_compile)
        {
            ...
            fprintf (fp, "\n#include \"GB_jit_kernel_ewise_full_noaccum.c\"\n") ;
